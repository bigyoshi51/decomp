...
| [POST_CC_RECIPES.md](POST_CC_RECIPES.md) | **MOSTLY DEPRECATED 2026-05-23.** Post-cc instruction-byte editing (INSN_PATCH / PROLOGUE_STEALS / instruction-appending SUFFIX_BYTES) was REMOVED as match-faking — see `feedback_no_instruction_forcing_matches_policy`. **A match = C compiles to target bytes.** When C can't, leave it `#ifdef NON_MATCHING`. Only genuine data/alignment mechanisms remain (all-zero padding SUFFIX_BYTES, USO-header PREFIX_BYTES, TRUNCATE_TEXT). For the cap-class previously fixed by INSN_PATCH (allocation/scheduling near-misses), use the permuter (see TOOLING_DECOMP.md). | Historical reference only for the removed mechanisms; consult for the surviving data-only entries. |
