...
- [PROLOGUE_STEALS successors must INLINE the stolen-prologue value's first use](#feedback-prologue-stolen-avoid-named-local-for-first-use) — **DEPRECATED 2026-05-23 (PROLOGUE_STEALS was removed as match-faking — see `feedback_no_instruction_forcing_matches_policy`). The "prologue-stolen successor" SHAPE recognition is still valid (a function whose first insns implicitly consume a predecessor's tail register-load) but the fix is no longer a splice — leave `#ifdef NON_MATCHING` / `#else INCLUDE_ASM` or do a focused-session splat boundary correction.** Historical content: _When a function used PROLOGUE_STEALS=8 (the predecessor's tail emits `lui $tN, 0; lw $tN, OFF($tN)` for this function), the FIRST use of that loaded value in C had to be inlined — not assigned to a named local.
...
- [Plain `switch(state){case K:..break;}` emits the target's `beq state,K,caseK` out-of-line dispatch where `if(state==0)else if(state==1)` emits in-line `bne`-skip blocks](#feedback-switch-emits-beq-dispatch) — _When the target dispatches on a value with `beq state,0,case0; beq state,1,case1; b end` (each case JUMPED to out-of-line, bodies after the dispatch, default falls to a shared end), the C should be a `switch`, NOT an `if/else if` chain. `if(state==0){A} else if(state==1){B}` compiles to sequential in-line `bne state,K,next; <body>` (bodies in-line, no top dispatch) — structurally different. `switch(state){case 0: A; break; case 1: B; break;}` produces the exact beq-to-forward-label dispatch + the default goto-end. Cracked h2hproc_uso_func_00000A88 77.4%->89.1% (2026-05-31) in one edit — no pad/goto needed (cf. the goto+pad combo below, which is for when a frame-size mismatch ALSO needs fixing). Recognition: 2+ `beq val,K` at the top before any case body._ **VERIFY case-merge before assuming fall-through:** when two adjacent case VALUES (e.g. 0x68/0x69) both appear in the dispatch, check whether their `beq` TARGET ADDRESSES are the same or DIFFERENT. Different targets = SEPARATE cases (each its own body), NOT a `case 0x68: case 0x69:` fall-through. A decode that merges them by value-adjacency is wrong — the bodies, calls, and even arg counts differ. Cracked gl_func_00054AEC 78.3%->89.8% (2026-05-31): 0x68 was `cb(strm, self+0x124)`, 0x69 the index-store, default `cb(self, strm)` — the merged decode had conflated all three.
...
- [objdiff fuzzy_match_percent dramatically overestimates byte-exactness on structurally-locked wraps — count word-diffs before assuming a near-match](#feedback-fuzzy-pct-overestimates-byte-exactness) — _A wrap doc citing 74.49% fuzzy match can have 57 of 59 word diffs (~3% byte-exact) when the divergence is structural (basic-block layout, jal ordering, epilogue shape). The title's old "before reaching for INSN_PATCH" framing is obsolete (INSN_PATCH removed 2026-05-23) — but the fuzzy-overestimate gotcha still matters: count word-diffs to decide between permuter (1-2 reg/imm diffs) vs accept-NM (structural diff)._
...
- [PROLOGUE_STEALS=8 splices off the FIRST 8 bytes — fragility note (DEPRECATED)](#feedback-prologue-stolen-function-shape-must-be-stable) — **DEPRECATED 2026-05-23 (PROLOGUE_STEALS removed as match-faking).** Historical: _PROLOGUE_STEALS was a blind 8-byte byte-offset splice from the start of the function's emit; any C refactor that changed IDO's prologue layout made the splice cut the wrong 8 bytes._
- [Before applying PROLOGUE_STEALS, verify the prefix is actually in the PREDECESSOR's symbol (DEPRECATED)](#feedback-prologue-stolen-misdiagnosis) — **DEPRECATED 2026-05-23 (PROLOGUE_STEALS removed as match-faking).** Historical: _A NM-wrap doc may claim "prologue-stolen successor — predecessor X ends with lui+lw setting tN=...".
- [Prologue-stolen PREDECESSOR class — SUFFIX_BYTES + PROLOGUE_STEALS combo (DEPRECATED)](#feedback-prologue-stolen-predecessor-no-recipe) — **DEPRECATED 2026-05-23 (PROLOGUE_STEALS + instruction-appending SUFFIX_BYTES removed as match-faking).** Historical: _Mirror of PROLOGUE_STEALS for the predecessor side.
- [Prologue-stolen SUCCESSOR — splice-function-prefix.py + Makefile PROLOGUE_STEALS (DEPRECATED)](#feedback-prologue-stolen-successor-no-recipe) — **DEPRECATED 2026-05-23 (PROLOGUE_STEALS removed as match-faking).** Historical: _Originally documented as "no recipe" (2026-05-01).
...
- [Param->local copies in decl order steer s-reg coloring on loop-carried values (9DB8 38.92->92.31)](#pattern-param-to-local-copies-decl-order-sreg-coloring) — _uopt colors: load-only pointer param s0 first, then LOCALS in decl order, then hoisted consts (li 5/li 36 -> s5/s7); copy params to locals + declare counter last = target s1..s4. Also: (x%8)<<3 decode gotcha; inline src[k] args for lw/sw pairing; dead kept-init move v0,zero irreproducible._
- [`while(1)+break` latch defeats IDO x4-unroll; volatile byte kills store-forwarding (2119C 38.86->91.95)](#pattern-while1-break-latch-defeats-ido-unroll) — _Multi-BB latch = uopt unroller refusal with identical latch codegen; volatile on loads/stores does NOT stop unroll; opaque bound = worse (remainder peel). Volatile global byte forces target-shaped per-case reloads; proxy held-base wins again on 2-s-reg fn._
- [Proxy-extern held-base WINS on single-s-reg targets; per-site scalar externs kill repeated-&D address-CSE; unroll-factor probe negatives (24378 38.75->69.43)](#pattern-proxy-extern-single-sreg-wins-plus-per-site-externs) — _Renumber penalty only bites with many s-regs; on a whole-fn $s0 held-base + 25 at-form singles fn, proxy lever + one-array-extern-per-scalar-site (explicit offsets keep baked %lo byte-exact) = +30pp. IDO x2-unrolls simple copy loops here; every hand-spelled remainder/x4 variant re-peels worse._
...
- [3-entry recipe combo for prologue-stolen-PREDECESSOR + bundled-TRAILER (DEPRECATED)](#feedback-three-recipe-combo-prologue-stolen-predecessor-plus-bundled-trailer) — **DEPRECATED 2026-05-23 (PROLOGUE_STEALS + instruction-appending SUFFIX_BYTES removed as match-faking).** Historical: _Some USO functions need THREE Makefile recipe entries to byte-match — SUFFIX_BYTES on the predecessor (injects stolen prologue for current func), PROLOGUE_STEALS on the current func (strips C-body's emitted prefix),…
...
- [Target RELOADS an indirect pointer (`lw rX, OFF(self)`) before each access-group — don't cache it in a C local; inline the deref + re-read per group](#feedback-reload-indirect-pointer-not-cache) — _When a function reads `p = self->field` then writes/reads several `p[i]`, and the asm shows `self->field` LOADED MORE THAN ONCE (e.g. `lw t6,68(a2)` for the first store + `lw v1,68(a2)` for the next RMW block), the source did NOT cache `p` — it re-read `self->field` per statement-group. The cached-`p` C is N insns short (one load where the target has several). Fix: write the early access inline (`((int*)self->field)[1] = v;`) and start a FRESH `p = self->field;` for the next block. Cracked gl_func_00060468 77%->88.2% (2026-05-31): split the cached `p` across the `p[1]=v` write and the `p[2]+=p[1]-p[0]` RMW. Recognition: same `lw OFF(base)` appears 2+ times for what your C treats as one cached pointer. (Distinct from struct-copy-recompute: this is a scalar pointer reloaded across groups, not a struct-copy address re-materialize.)_
...
- [A loop-guard `blez/beqz` whose branch TARGET is a post-loop TAIL block (not the epilogue) means that tail is UNCONDITIONAL — drop the `if(count<=0) return` guard; the for-loop's own `i<count` test is the single branch](#feedback-guard-branch-targets-tail-not-epilogue) — _Before writing `if(count<=0) return;` ahead of a fill/count loop, trace where the `blez count` branch LANDS. If it targets a block of real work near the end (a final store/init) rather than the `jr ra` epilogue, that block runs even when count<=0 — it's UNCONDITIONAL post-loop code, not skipped. Two fixes: (1) add the missing tail block after the loop; (2) drop the explicit `if(count<=0) return` / `if(count>0){...}` wrapper — a bare `for(i=0;i<count;i++)` emits ONE `blez` (0 iterations when count<=0, then falls straight into the tail), where the wrapper emits a DUPLICATE blez and misaligns IDO's 4-way-unrolled fill. Cracked gl_func_0004ACD4 85.6%->99.3% (2026-05-31): missing unconditional `self->0xD4 = ...` tail + removing the redundant count guard. Recognition: `blez count, L` where L disassembles to `+0xN` landing on non-epilogue insns._
- [`or sN, a0, zero` in the FIRST call's delay slot (then reads `sN->field`, ignoring v0) = `self` IS the param, not the call return — `self=arg0; func(arg0);` not `self=func(arg0)`](#feedback-param-as-self-discarded-return) — _When a function opens with `jal func1; or sN,a0,zero` (the move is the jal's delay slot) and then dereferences sN (NOT v0), the first call's return is DISCARDED and `self` is the incoming parameter, established in a callee-saved register via the delay-slot move. The wrong C `self = func1(arg0)` keeps the result in v0 and stack-spills it across later calls (extra insns); the right C is `func1(arg0); self = arg0;` (call for side-effect, self = the param). This also flips the regalloc: self promotes to sN (no spill) because the param is movable into a saved reg BEFORE the call returns. Cracked gl_func_00061E9C 70.3%->100% (2026-05-31): combined with address-style assert arg (&D+0xN -> lui+addiu not literal lui+ori) + inlining a single-use temp to color it tN-high. Recognition: first-call delay slot is `or sN,a0` + body reads sN not v0._
...
**C form (BYTE-EXACT, 2026-06-20 — supersedes the "residual cap" below):** declare the function as **actually variadic** — `int *fn(int *a0, int a1, ...)`. The `...` is what reserves the extra +8 frame AND homes a1/a2/a3 to the caller shadow slots; you do NOT need `&a2`/`(void)a3` tricks. The scratch = the va_list cursor at a2-home: `aligned = (((int)((char *)&a1 + 4)) + 3) & ~3;` → emits `or a2,sp; addiu 0x33; and ~3` (the `+4` steps past `a1` to a2-home; `+3 &~3` is the 4-byte align-up). **Use the PARAMETER `a0` directly** (`if (a0==0){ a0=alloc(N); … } … return a0;`) — NOT a fresh `s0` local — so IDO schedules `or s0,a0` early and `sw a3` into the `bne` delay slot, matching the target's prologue order exactly. (gl_func_0004C190: 31/31 words, 0 non-reloc diffs.)

**~~RESIDUAL CAP~~ (FALSE — was a MISDIAGNOSIS, cracked 2026-06-20):** the prior note claimed the target "bases the scratch at sp+0 inside a frame 8 bytes larger… an IDO allocator artifact needing a donor splice." That was wrong. The `or a2,sp` is just `a2 = sp` *before* the `addiu 0x33; and ~3` walks it up to a2-home (sp+0x30) — the final scratch pointer is a2-home, identical to the no-local form, just at a larger frame. And the +8 frame is NOT an artifact: it is the **variadic frame reservation** that a real `...` declaration produces. `__builtin_alloca` failed because alloca is a red herring; the answer was declaring the function variadic. **Lesson: when a varargs-shaped constructor's only residual is a +8 frame (and the scratch is `or aN,sp; addiu K; and ~3` = a vararg-home pointer), try a real `...` declaration before conceding a "frame-size / donor-splice cap."**
...
- **CRACKED 2026-06-23 (agent-d) — the "standalone caps at 74%" framing was the trap.** The fix is NOT to compile `func_800081D0` standalone (which always emits a prologue) — it is to compile the WHOLE logical function in ONE TU (kernel_040.c `func_8000817C`) so the continuation bytes are real C, then expose the mid-function entry as a section-relative linker symbol:
  1. Write the full `__rmonClearBreak` body (both blocks) as `func_8000817C`. Apply the named-value-local + dead-slot-coalesce levers so the SECOND block's BREAK-test value reuses the FIRST block's spill slot (`0x1C(sp)`): use ONE `s32 instr_a` local for both blocks (lifetimes disjoint → IDO coalesces). A separate `instr_b` local gives slot `0x18(sp)` = 1 diff word.
  2. Extend the owning .o's `TRUNCATE_TEXT` to cover the full function (0x100 → 0x194) so the continuation isn't clipped away.
  3. Remove the continuation's INCLUDE_ASM fragment from the sibling .c (here kernel_020_c.c) so it doesn't duplicate/displace; delete the orphaned `asm/nonmatchings/.../func_800081D0.s`.
  4. Expose the mid-function entry label in the linker script INSIDE the `.text` output, **section-relative**: `func_800081D0 = func_8000817C + 0x54;`. (An absolute `= 0x800081D0` makes R_MIPS_26 jal relocs from callers fail "relocation truncated to fit" — it must be section-relative.)
  Result: full ROM cmp-clean vs baserom; all 37 continuation words byte-exact. This recipe generalizes to any no-prologue mid-function alt-entry whose predecessor is in the same TU. DO NOT merge-fragments (still breaks the jal symbol) — the linker-symbol alias is what preserves it.
...
**Unwritten-reserve variant — `volatile char _pad[N]` for a stack region nothing writes (gl_func_000628EC, 2026-05-31):** some functions reserve a large stack block (here ~96 bytes, frame 0x98 vs the 0x38 a naive decode gives) that NO instruction reads or writes — the original source had a local (big struct / sprintf buffer / array) used in a way the partial decomp can't reconstruct. A plain unused `char _pad[N]` is dead-store-eliminated (GCC -O2 frees the slot); making it `volatile char _pad[N]` forces GCC to reserve the N bytes WITH NO STORES (volatile-unused = space reserved, zero instructions emitted), matching the target's unwritten region. This reconstructs the frame size + realigns the incoming-arg homes and high spills, partially solving the "target reserves unused stack, unreconstructable" cap. Caveats: (1) the metric may barely move if objdiff weights the offset shifts low and the real residual is a codegen cap — it's still worth it for frame correctness; (2) it won't nail every low-frame spill offset (the real reserve's internal partition is unknown), so a few 4-byte spill-slot diffs can remain. Distinct from `volatile int` spill-forcing (which OVER-grows the frame, see IDO_CODEGEN anti-pattern) — the volatile ARRAY of the exact byte size is the controlled lever.
...
**Picking a candidate (positioning matters):**
- A **leaf return-value/move/const** function (`return 0`, `return x`, `*p`) genuinely needs the lower opt: `-O2` fills the jr-delay, `-g3`/`-O0` don't. These are the real targets.
- **Framed functions ending `addiu sp; jr ra; nop`** (e.g. a 2-call wrapper) are usually `-O2`-naturally-unfilled — they match at plain `-O2` and need NO split (just un-INCLUDE_ASM them in the main file). Don't mistake them for `-g3` candidates; test `-O2` first.
- **Cleanest split = 2-way** when the carved region is at the **start or end** of the USO (main file just truncates the other side). A mid-USO target needs a 3-way split (before / sub-unit / after-file) or a sandwich (sub-unit spans to end with interspersed `-O2` funcs as verbatim `INCLUDE_ASM`). `mgrproc_uso.c` happens to open with a contiguous `-O0` run (func_00000000/4C/B0) — a prime batch for one sub-unit.
- **A single mid-block function = binary-slice, NOT a 3-way source split (cleanest, 2026-05-29).** When exactly ONE mid-block function needs the lower opt level (e.g. an isolated unfilled-jr-delay `return 1` surrounded by filled-delay matches), don't split the 5000-line bulk file into before/after halves. Instead: (1) **delete** the function from the bulk `.c` entirely (no `INCLUDE_ASM`, no body — just remove it; the bulk `.text` is now `0xSIZE` shorter and everything after it shifts up); (2) compile the lone function in its own `-O2 -g3` (or `-O0`) sub-unit `.c` with `TRUNCATE_TEXT := <size>`; (3) in the Yay0 rule, objcopy BOTH `.text`s and **slice the bulk `.text` at the function's original offset, inserting the sub-unit chunk between the halves**, trimming the result back to the bulk's original length (the bulk gains exactly `<size>` of trailing 16-align zero-pad after the delete, which the trim drops):
  ```python
  # off = func's original block offset; g = carved sub-unit .text (size bytes)
  m=open(main_bin,'rb').read(); g=open(g3_bin,'rb').read()
  tail=m[off:]; assert tail[-len(g):]==b'\x00'*len(g),'main tail pad not zero'
  open(out,'wb').write((m[:off]+g+tail)[:len(m)])
  ```
  Verify the spliced `.text` is **byte-identical to the pre-change snapshot** (it will be — the carved bytes equal what `INCLUDE_ASM` emitted, just relocated to a compiled sub-unit so it counts as a real match). objdiff scores the sub-unit per-function (target=expected, base=non_matching, both build the same `-g3` C → 100%). Ref impl: `timproc_uso_b5_g3_1DA4.c` + the `timproc_uso_block5_yay0.bin` rule (landed `timproc_uso_b5_func_00001DA4`). This avoids touching the bulk file's other ~150 functions and needs no source-level before/after carve.
- Always verify per-function: `-O2` build ≠ target AND `-O0`/`-g3` build == target (jal fields reloc-resolved).

**Non-zero-`%lo` data refs need a reloc-bake before the Yay0 objcopy — SOLVED 2026-05-29 (`scripts/bake-data-relocs.py`).** A C body referencing `&D_<off>` with a non-zero `%lo` (e.g. a fused `lui at; sw %lo(D_0000014C)(at)` store) **matches objdiff** (reloc-aware) but `objcopy -O binary` extracts the `.text` *pre-link*, leaving the LO16/HI16 raw — the blob had `0x0000` where the ROM has the baked `%lo`. Opening-run functions matched byte-identically only because their refs were `%lo=0` (`&D_00000000+0`); mid-file `-O0` functions reference `&D+offset` globals and hit this. **Fix (implemented):** after each region's `objcopy`, the Yay0 concat rule runs `scripts/bake-data-relocs.py <region.o> <region.text.bin> undefined_syms_auto.txt`, which applies R_MIPS_HI16/LO16 from the `.o`'s reloc table (symbol values from `undefined_syms_auto.txt`, e.g. `D_0000014C = 0x14C`) into the raw `.text`, leaving **R_MIPS_26 jals raw** (the USO ships jals unrelocated / fixed at load). It's a **no-op for `%lo=0` refs** so it doesn't disturb existing splits. First use: `mgrproc_uso_func_000009A8` landed (`-O0` int cleanup via `register int z=0` + `D_0000014C`; its `0x14c` `%lo` now bakes into the blob → byte-identical). This unlocks the mid-file `-O0`/`-g3` vein across the Yay0 USOs (each still needs per-function `-O0` RE). NOTE: the bake only touches the blob, never the `.o` objdiff scores against, so a bake bug can't regress the match metric.

---

### Historical blocker write-up (pre-2026-05-28)
...
**Generalizes to any if/else with a duplicated common-tail store** (not just alloc cascades). If both arms of an `if/else` end with the SAME store to the SAME location — e.g. `if(c){ extra; cur=recompute; cur[X]=V; } else { cur[X]=V; }` where the recompute yields the same pointer — that store is a SHARED TAIL: hoist it to a single statement AFTER the `if` (drop the `else`). The tell-tale that you have this (vs a register-cascade over-production) is a redundant **`beq zero,zero`** (unconditional goto) in the BUILT code that the target lacks: the duplicated arm forces IDO to branch over the other copy. Collapsing it removes that goto and matches the target's instruction count. Took game_libs_func_00003F08 88.5%→90.7% (60 insns, size-matched; residual was the target's `beql` delay-fold idiom + a pointer register cascade — separate caps). WORKFLOW: after `scripts/find-size-mismatch.py` flags an OVER (+delta) function, `objdump` the built body and grep for a `beq zero,zero` (or `b <addr>`) absent from the target — present ⇒ shared-tail collapse; absent ⇒ probably a register-cascade/spill cap (leave it). Confirmed 2026-05-30. REFINEMENT (2026-05-30): the `beq zero,zero` signal is necessary but NOT sufficient. Two sub-cases: (a) a SINGLE common store duplicated across an if/else (cleanly collapsible — game_libs_func_00003F08, +3→size-matched); (b) an ALLOC-CASCADE constructor whose `if(!x){alloc;if(!x)return 0;}` failure arms each emit a goto (h2hproc_uso_func_00000EB0 +4, titproc_uso_func_00002980 +2). Case (b) is HARDER: changing `return 0`→`return p` is byte-neutral, and converging the failure arms intertwines with branch-polarity (`bne` skip-return vs target `beq` do-return) AND the p/q pointer register roles — those don't collapse from the convergence rewrite alone (attempts left them +2/+6, no better). So only pursue case (a): confirm the extra `beq zero,zero` guards a DUPLICATED STORE (same dest in both arms), not an alloc-failure early-return. The constructor convergence DOES work when the only diff is the goto target (n64proc_uso_func_00000100 88→95%), but fails when register roles also differ. UNDER (-delta) constructors are even harder: a missing alloc-PASSTHROUGH guard (`sub = root; if(sub==0){sub=alloc;...}`) often CANNOT be re-added from C, because IDO proves `root != 0` (it survived a checked `if(root==0){root=alloc;if(root==0)goto end;}`) and propagates that through `sub=root`, optimizing the dead `if(sub==0)` away — so the built code stays short (titproc_uso_func_00000C54 stuck at 90%/-3; `sub=root;if(sub==0)` was byte-neutral). The target emits the dead guard anyway (IDO's per-function path-insensitivity differs case to case). No reliable C lever to force the dead guard; leave these UNDER constructors. NET on the size-mismatch vein: the reliably-winnable subset is small (clean if/else duplicated-store collapse + goto-only convergence); register-role swaps, branch-polarity, and provably-dead-guard cases do not yield to C rewrites.

## `or sN, a0, zero` in the first call's delay slot = `self` IS the param, not the call return {#feedback-param-as-self-discarded-return}

_When a function opens `jal func1; or sN,a0,zero` (the move fills the jal delay slot) and the body then dereferences `sN` while NEVER reading `v0`, the first call's return value is DISCARDED and `self` is the incoming parameter — moved into a callee-saved register before the call even returns._

```
# target — gl_func_00061E9C opening
addiu sp,sp,-40
sw   ra,36(sp)
sw   s0,32(sp)
jal  func1
or   s0,a0,zero      # delay slot: s0 = arg0 (the PARAM, not v0)
lw   t6,20(s0)       # reads s0->0x14, ignores func1's v0 return
```

Wrong C (keeps result in v0, stack-spills across later calls — 2 extra insns):
```c
int *self = func1(arg0);     // self = v0 -> spilled
```
Right C (call for side effect, self = the param):
```c
func1(arg0);                 // return discarded
int *self = arg0;            // promotes to s0, no spill
```
...
Cracked `gl_func_00038728` 41.2%→64.4% and `gl_func_00038964` 52→58% (2026-05-31) after first mis-reading them as nested list-of-lists. **CAVEAT (verified 2026-05-31): the rotated-prefetch C form is function-specific — it REGRESSED the smaller siblings `gl_func_00038598` (51.4→46.1) and `gl_func_00069B94` (52.5→51.3), where the plain `while(node){ obj=node[0]; ...; node=node[1]; }` form is closer. ALWAYS measure objdiff before/after and revert if it drops; do NOT blanket-rewrite the whole family to the rotated form.** **Rule:** when a `v0 = mem` load looks dead because an unconditional `beq zero,zero` jumps over it, check whether it's the branch's delay slot — if so it runs, and you're looking at a rotated single loop with delay-slot loads, not nesting. (Residual after the logic fix is usually regalloc: the target spills the cursor/next chain to stack slots while IDO keeps them in saved regs — a permuter target, not a logic gap.)

## USO draw/cb decode bugs: literal `0` first-arg should be `&D_00000000`, and out-param stack buffers must be arrays {#feedback-uso-drawcall-zero-arg-and-array-dce}

Two recurring m2c/DCE bugs in relocatable-USO draw-step functions (verified 2026-05-31 on `mgrproc_uso_func_00001C90`, 88.1% → 98.68%):

**(1) Literal `0` first-arg that should be `&D_00000000`.** When the build emits `or aN, zero, zero` (aN = literal 0) but the target emits `lui aN, 0x0; addiu aN, aN, 0` (a reloc'd `&D_00000000 + 0`), the call's argument is the **data-base draw context pointer**, not 0. m2c writes `0` because in the relocatable USO the resolved address of `&D+0` is literally 0x0 (the data section base), so the `lui;addiu` looks like "load address 0." Fix: pass `&D_00000000` (or `(char*)&D_00000000`) instead of `0`. Recognition: `or aN,zero,zero` in build vs `lui aN; addiu aN` in target at a call-arg-marshaling site. Recurs at every draw call in the function (here 4 sites, +2pp). Distinct from a genuine `0` arg (which stays `or aN,zero,zero` in BOTH).

**(1b) Literal `N` arg that should be the ADDRESS `&D_00000000 + N`.** Same m2c reloc-blindness for a non-zero `&D`-relative address arg: build emits `addiu aN, zero, N` (literal N) where target emits `lui aN, 0x0; addiu aN, aN, N` (= `&D + N`). Fix: pass `(char*)&D_00000000 + N` instead of the literal `N`. Recognition: `addiu aN,zero,N` (build) vs `lui aN; addiu aN,aN,N` (target) at a call site. Verified 2026-05-31 on `mgrproc_uso_func_00002940` (the `0x650` cb arg, part of an 83.0→88.59% pass that also used the distinct-extern &D-CSE bust on the `&D+0x68` value-read).

**(2) Out-param stack buffer must be an array, or GCC DCE's the elements it can't see read.** A stack Vec/quad initialized then passed by `&buf[0]` to a K&R callee: GCC sees only `&buf[0]` taken, can't see the callee reads `buf[1..N]`, and **dead-store-eliminates** the writes to `buf[1..N]` (target keeps all N stores). Declaring the buffer as an array (`float sp50[4]` instead of `float sp50,sp54,sp58,sp5C;`) and passing the array name makes the whole object address-taken, blocking per-element DCE — all N stores survive. Recognition: target has N adjacent `swc1 $fX, k(sp)` / `sw rX, k(sp)` stores to a contiguous stack block but the build emits only the first (the one whose address is passed). +2.6pp here. (Note: this is DCE-prevention, distinct from the oversized-array frame-size lever and the 1-element-array spill-forcing lever — same `T arr[N]` syntax, different mechanism.)

These two + the distinct-extern &D-CSE bust (see `feedback-ido-cse-bust-via-distinct-externs`, applied to the same function's ramp float-consts, +5.9pp) together took 1C90 from 88.1% to 98.68% in one pass. When auditing a USO draw/cb orchestrator NM-wrap, check all three before declaring caps.

## A loop "dead cursor" that increments but is never read is usually the per-iteration READ BASE (decode bug) {#feedback-dead-loop-cursor-is-read-base}

When a do/while loop has a pointer local that increments each iteration (`p += 4`) but the decoded C never reads through it — and the per-iteration body reads a FIXED base (`*(T*)(a0 + OFF)`) instead — the "dead" cursor is almost always the real read base, and the decode is wrong. The target reads `*(T*)(p + OFF)` (cursor + offset), so each iteration touches a different slot; GCC -O2 then DCEs the truly-dead `a0`-based version's cursor, and the build is short a saved register + a smaller frame.

**Signal:** target asm has `lw vN, OFF(sCURSOR)` where `sCURSOR` is a saved reg that gets `addiu sCURSOR, sCURSOR, 4` (or 8…) each iteration, AND the build keeps the loop base in a register that never increments. The build is also typically 1 saved-reg / 8 frame bytes short (the cursor lost its home).

**Fix:** read through the cursor — `v = *(T*)(p + OFF)` instead of `*(T*)(a0 + OFF)`. This makes the cursor live (it gets its own saved reg, frame grows, loop matches). Then set the init order to control the i-vs-cursor saved-reg assignment (`i = 0; p = a0;` → `i`→`$s0`, `p`→`$s1`).

Verified 2026-05-31 on the `timproc_uso_b5_func_0000A97C/A9EC/D06C/D0DC` template family (4 sibling loops, identical shape, different fn-table offsets): each 72.21% → 98.75% from this single decode fix (the old A97C comment had explicitly mis-labeled the cursor "unused/dead but live in IDO's view" — it was the read base all along). Residual on all four is just the v0/v1 caller-save preload swap (branch-likely delay-slot loads the cursor-read into `$v1`; IDO-emit uses `$v0`) — a register-renumber cap. **Template-family leverage:** when several functions share a fuzzy% AND size, they're usually one shape — fix one, batch-apply to the rest.

### jr-rodata switch DISPATCH reproduces for single-symbol USO functions — "can't reproduce" is wrong (2026-06-01)

Many USO functions are 11/12-way state-machine dispatchers (`sltiu at,a1,N; sll; lui; addu; lw t,table(at); jr t`) and were marked permanent-INCLUDE_ASM with "the .rodata jumptable is discarded by the USO linker, so the jr-via-rodata dispatch can't be reproduced" (citing feedback_ido_switch_rodata_jumptable). **That blocker is FALSE for a single-symbol function.** Write the logic as a C `switch(state)` with dense cases 0..N-1: IDO emits the exact `sltiu/sll/lui/addu/lw/jr` dispatch in `.text`, and objdiff compares only `.text` — the generated `.rodata` jumptable is a separate section it doesn't score. The dispatch + every correctly-decoded case body match. Proven: arcproc_uso_func_00000240 0.88%→22.22% just by writing the do-while + 11-way switch (cases 0..4 real, 5..10 as distinct `D[0x40]=N` placeholders to force the dense table); same mechanism as timproc_uso_b5_func_000087F4. 

Recipe: (1) `if ((unsigned)state >= N) break;` reproduces the `sltiu;beqz` bound check; (2) write ALL N cases with DISTINCT bodies (a unique store/return) so IDO keeps the dense jump-table form rather than collapsing to an if-chain — even undecoded cases need a distinct placeholder; (3) fill real case bodies incrementally (multi-tick), each adds %. The ONLY time the jr-rodata genuinely blocks is when the case bodies are SEPARATE splat symbols (boundary defect — see MATCHING_WORKFLOW byte-correct-switch-0% entry); a single symbol has no such issue. Action: grep for NM-wrap comments citing "jr-via-rodata can't be reproduced" and re-attempt them as switches — likely a large unlocked cohort across arcproc/titproc/mgrproc/gui USOs.

### jr-rodata switch CAVEAT: convert if-chain→switch only when case bodies are COMPLETE (2026-06-02)

Refinement of the switch-dispatch-reproduces entry above: the `switch` form wins ONLY when the case bodies are fully decoded. If a dispatcher is partially decoded (if-chain with incomplete case bodies), converting to `switch` can REGRESS the fuzzy score even though the switch's `.text` dispatch is the "correct" structure — because the incomplete bodies after the dispatch misalign more than the matched dispatch helps (cascade-realign: the dispatch is early, body divergence dominates). Verified on titproc_uso_func_0000116C: if-chain 40.32% → switch 37.1% (reverted; 143 case-body insns still missing). Rule: finish the case bodies FIRST (keep the if-chain as a scaffold), then flip to `switch` as the last step. A fresh/empty dispatcher (like arcproc_uso_func_00000240) goes straight to switch because there are no half-aligned bodies to lose.
...
**CORRECTION / FRAGILITY (2026-06-03, A7F8):** the `if (p == &buf, p == 0) alloc` dead-branch emission is UNRELIABLE — IDO folds `&local == 0` to false inconsistently. Measured: A3C4 emits **2 of 3** alloc sites (`addiu a0,12`: built 2 / exp 3), but A7F8 emits **0 of 4** (built 0 / exp 4) with the identical `Vec3 *p=&buf; if(p==0) p=func(12);` form. So do NOT count on the dead `alloc(12)`/`jal` reproducing. What DOES reliably help is the **member-access shape**: switching the buffers from `float b[3]`/`float*` to `Vec3`/`Vec3*` so fills emit via `.x/.z/.y` in the target's load-store order (A7F8 +3pp from this alone, allocs absent). The target's dead-allocs are almost certainly an inlined vec-init helper (`f(out){ if(!out) out=alloc(12); ... }`) where IDO inlines the body BEFORE the const-fold pass that would kill `&buf==0`; a top-level hand-written `if(p==0)` runs after that pass and gets folded. We can't add a noinline helper, so treat the missing alloc sites as a residual cap, not a bug to fix. Net A7F8 lever this session: 36.1→47.5% (opening struct-copy chain + Vec3-ifying the buffers); the 4 absent allocs + spill-slot RA (frame −208 vs −256) are the remaining non-C-forceable gap.

### Gotcha: game_uso "big insn-count gaps" are usually RA/shape divergence, NOT missing logic (2026-06-02)

When a game_uso NM wrap is N insns short of target, do NOT assume a missing logic block. A normalized difflib of built-vs-expected renders pervasive RA divergence (temp renumbers, `(x&m)!=0` booleanization via `sltu`, pointer materialization, redundant reloads) as large fake `--delete`/`--insert` regions that LOOK like missing switch/branch blocks. Verify by reading the actual C against the asm first: e.g. game_uso_func_0000E5C8 showed a "missing 22-insn switch on a3->0xE8" in the diff, but that exact switch (cases 5/6/2/1/4/3 → |=0x20000/0x10000/0x4000/0x1000/0x8000/0x2000, gated by a3->0x12C) was ALREADY in the C body — the 17-insn shortfall is IDO emitting more insns for the target's shape (booleanize-and-store of flag tests, reload-per-use) than the natural C triggers. These are RA/shape caps, not decodable gains. The genuinely-decodable game_uso gaps were the exceptions: 6FA8 (un-decoded RETURN mechanism) and A3C4 (dead-alloc/stack-buffer idiom). Confirm "missing logic" by symbol/struct-offset reasoning, not by trusting the diff's block boundaries.
...
<a id="pattern-proxy-extern-single-sreg-wins-plus-per-site-externs"></a>
## Proxy-extern held-base WINS when the target uses only ONE s-reg; pair it with per-site scalar externs to kill repeated-&D address-CSE (24378, 38.75->69.43)

_The known renumber penalty of the link-time-0 proxy extern (`B = (char*)sym + (int)&proxy;`) only bites on many-s-reg functions. On a whole-fn held-base target (only $s0 saved, ~25 `off($s0)` accesses, 20+ calls), the proxy lever converts every rematerialized absolute `lui at; op K(at)` pair into the target's single `op K(s0)` form — net +30pp on gl_func_00024378 (1488B, 38.75->69.43). Cost is only the 3-insn `lui+lui+addu` prelude vs the target's 2-insn `lui+addiu`._

**Companion spelling rule (per-site scalar externs):** when the SAME fn also has many at-form singleton accesses to `&D_00000000+small` (stores/loads at +0, +4, +0xC, float table +0xEC8..), spelling them all as `(char*)&D_00000000+off` makes uopt address-CSE them into a held caller-saved base (a2) — target has independent `lui at` per access because the original had DISTINCT symbols. Fix: one array extern PER SITE with the offset spelled explicitly (`extern u8 gl_d_x_a[]; *(s32*)(gl_d_x_a + 4)`). All USO symbols resolve to 0, so the baked %lo immediates stay byte-exact while distinct relocs make CSE impossible. ~25 sites converted this way on 24378, each landing target-shaped.

**Unroll probe (negative results, don't re-probe):** the target's andi-3 remainder + x4 copy loop could NOT be reproduced: simple indexed loop -> IDO x2-unrolls (andi-1); stepped-index locals -> still x2; hand-spelled m2c remainder(do-while,!=) + x4 main -> remainder gets x2-peeled (+30 insns); counted-for remainder + x4 main -> remainder x2+peel AND main x8 (worst). Simple indexed loop is the closest overall shape (fewest total insns, best fuzzy). The x4-with-clean-remainder factor choice remains an open cap for this band.

<a id="pattern-while1-break-latch-defeats-ido-unroll"></a>
## `while(1){...; if (j == N) break;}` latch form DEFEATS IDO -O2 constant-trip x4-unroll (volatile does NOT); volatile state-byte kills store-to-load forwarding (2119C 38.86->91.95)

_When the target has a small NON-unrolled constant-trip copy/clear loop (e.g. stride-8, four `sh zero` per iter, `bne j,bound` latch) but every `do{...}while(j!=N)` / `for` spelling gets x4-unrolled (16 stores, stride 32), rewrite the latch as `while (1) { stores; j += N_step; if (j == N) break; }`. The early-exit `if` makes the loop body multi-BB, which uopt's unroller refuses, while the emitted code is IDENTICAL to the do-while latch (`addiu j; bne j,bound` with the last store in the delay slot). Verified on gl_func_0002119C case-4 clear loop: 16 `sh` -> 4 `sh`, function 73.1 -> 91.9._

Negative probes on the same loop (don't re-probe):
- `volatile` on the loads AND/OR stores inside the loop: unroller ignores volatile entirely (still x4).
- Opaque bound via link-time-0 extern (`j != (int)lim_sym + N`): trip becomes unknown -> WORSE (andi remainder pre-peel + x4 main, the documented unreachable-cap shape).

**Companion (same fn): volatile global byte kills store-to-load forwarding.** When target RELOADS a global byte after decrementing it (per-case `lbu` before the common return expr; multiple `b`-to-epilogue paths carrying `lbu` in delay slots) but your build forwards the just-stored value (`andi v0,vX,0xff` + precomputed `slti` per path), access the byte through `*(volatile unsigned char *)` at EVERY site (a macro helps). Store order stays schedulable into delay slots; each read becomes a real `lbu` like the target. Also: proxy-extern held-base confirmed a WIN again on a 2-s-reg fn (s1=base, s0=loop cursor; no-proxy A/B = 65.4 vs 91.9 — const-fold to at-form dwarfs the renumber penalty).

<a id="pattern-param-to-local-copies-decl-order-sreg-coloring"></a>
## Param->local copies in declaration order steer uopt s-reg coloring on loop-carried values (9DB8 38.92->92.31)

_When ALL loop-carried values (params + counter) land in s-regs but SCRAMBLED vs target (the classic "correct-logic/divergent-regalloc" cap), copyeach param into a LOCAL and order the declarations to match the target's s-reg sequence. Observed uopt coloring order on gl_func_00009DB8: the pointer param used only via loads (a3/src) colors s0 FIRST regardless, then LOCALS in declaration order (op->s1, va->s2, vb->s3, i->s4), then loop-hoisted constant materializations (li 5 -> s5, li 36 -> s7), with a later-declared address local (ctx) slotting s6. Params left un-copied color AFTER locals — so a local counter declared before un-copied params steals the wrong s-reg; copying the params to locals (va=ia; vb=ib;) and declaring the counter LAST of the carried set fixed s1..s4 in one step (65-insn fn: loop body + epilogue became byte-exact including t-reg ids)._

Companions on the same fn: inline `src[0..2]` directly as call args (locals batch the lws; inline args give the target's interleaved lw/sw-argslot pairing); order the increment statements so the target's delay-slot increment is LAST in C (src+=3 emitted into the latch delay). Decode gotcha: the `bgez x; andi r,x,7; beqz r; addiu r,-8; sll r,3` cluster is plain truncated `(x % 8) << 3` applied UNCONDITIONALLY — not an "ib<0-only adjust" (the >=0 path still adds (x&7)<<3). Residual cap: a dead pre-header `move v0,zero` (kept init of a copy-propagated z=0 arg) does NOT reproduce from `int z = 0;` — IDO DCEs it.
