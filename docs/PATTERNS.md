...
**⚠ KNOWN LIMITATION — non-zero-`%lo` data refs don't bake into the Yay0 blob (2026-05-28).** A C body that references `&D_<off>` with a non-zero `%lo` (e.g. a `lui at; sw 0xN(at)` fused store to `D_0000014C`, where `0xN = %lo`) **matches objdiff** (reloc-aware resolves the LO16 to the symbol value) but the **Yay0 blob diverges**: `objcopy -O binary` extracts the `.text` *pre-link*, so the LO16/HI16 relocations are NOT applied — the blob has raw `0x0000` where the real ROM has the baked `%lo`. The opening-run functions matched byte-identically only because their data refs were `%lo=0` (`&D_00000000+0`). This blocks clean (byte-identical-blob) landing of most mid-file `-O0` functions, which reference `&D + offset` globals. **Verified** on `mgrproc_uso_func_000009A8` (`-O0` int cleanup): full RE matches at objdiff (fuzzy=100) via `register int z=0` + `D_0000014C`, but the blob diverges by exactly 2 bytes (the `sw`'s `0x14c` → `0x0000`). **Fix (not yet implemented): a `bake-data-relocs` step before the Yay0 objcopy** — apply R_MIPS_HI16/LO16 from the `.o`'s reloc table (symbol values from `undefined_syms_auto.txt`, e.g. `D_0000014C = 0x14C`) into the `.text` bytes, leaving R_MIPS_26 jals raw (USO ships jals unrelocated / fixed at load). This is a no-op for `%lo=0` refs (so it won't disturb the landed splits) and would unlock the whole mid-file `-O0`/`-g3` vein across all Yay0 USOs.
