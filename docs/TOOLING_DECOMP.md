...
- [`import.py` "Failed to find compile command" on a big multi-fn TU → build the permuter scratch dir BY HAND (base.c=cpp-expanded source, target.o=as-assembled .s, compile.sh, settings.toml); + reseed-from-best chains a frame-cascade near-miss up ~0.1-0.2pp/pass but plateaus shy of 100](#feedback-permuter-manual-scratch-and-reseed-chain) — _`import.py` derives compile flags by running `make --always-make --dry-run` and grepping for the source filename; on a 30k-line shared TU (e.g. `game_libs_post0b.c`) where you'd rather permute a minimal extract, or any scratch .c that isn't itself a make target, this fails with "Failed to find compile command from build script output". Bypass: construct `nonmatchings/<name>/` manually — `base.c` = `cpp -nostdinc -DNON_MATCHING -I include -I src scratch.c | grep -v '^#'` (preprocessed, IDO cc has no header path issues then), `target.o` = `mips-linux-gnu-as -EB -march=vr4300 -mabi=32 -I include` over the `.s` with the glabel/endlabel macro prelude prepended (copy the prelude from any existing scratch's `target.s`), `compile.sh` (cd to project root + `tools/ido-static-recomp/build/7.1/out/cc -c -G 0 -non_shared -Xcpluscomm -Wab,-r4300_mul -O2 -mips2 -32 -I include -I src "$INPUT" -o "$OUTPUT"`), and `settings.toml` (`func_name`, `compiler_type="ido"`). Verify the manual base.o is byte-identical to the in-tree `build/non_matching/.../X.c.o` before trusting the base score. Then the RESEED-FROM-BEST chain: run `--best-only --stop-on-zero`, take the best `output-NNN-K/source.c`, make it the next scratch's `base.c`, re-run. On a frame-layout/coloring cascade (gl_func_0004C928: frame 0x80 vs target 0x78) each reseed found a fresh dead load-bearing artifact (`long long pad;`/`int new_var3;` spill-slot decls, dead `if(arg0->dD0){}`/`do{}while(0)` blocks, `((0,arg0))` comma-expr, `new_var3=0x1C4` offset indirection) that nudged v0/v1 coloring one notch: permuter score 720→610→595→560→515→490, objdiff fuzzy 97.56→98.56% over 4 reseeds. It did NOT reach 100 — the +8-byte frame is a genuine optimizer spill (verified INVARIANT to declaration order via a 120-permutation full-decl-order brute force), an entangled frame-shrink+coloring fix the permuter's randomization floors at. IMPORTANT: the permuter score is NOT objdiff fuzzy — splice each candidate in-tree and `objdiff-cli report generate` to read the real `fuzzy_match_percent` (and remember to `git checkout HEAD -- report.json` after, since report generate stomps the tracked file). 2026-06-22 agent-b._
...
### `import.py` fails on a big multi-fn TU → build the scratch dir by hand; reseed-from-best chains a frame-cascade near-miss up but plateaus (2026-06-22)
...
**Reseed-from-best chain** (the lever that gained the most here): run `-jN --best-only --stop-on-zero`, take the best `output-NNN-K/source.c`, make it the NEXT scratch's `base.c`, re-run; repeat. On gl_func_0004C928 (a frame-layout + v0/v1-coloring cascade, frame 0x80 vs target 0x78) four reseeds drove permuter score 720→610→595→560→515→490 and objdiff fuzzy **97.56→98.56%**. Each pass discovered a fresh DEAD load-bearing artifact (`long long pad;`/`int new_var3;` spill-slot decls, dead `if(arg0->dD0){}`/`do{}while(0)` blocks, `((0,arg0))` comma-expr, `new_var3=0x1C4` offset indirection) that nudged the coloring one notch. It did NOT reach 100: the +8-byte frame is a genuine optimizer spill — verified INVARIANT to declaration order by a 120-permutation full-decl-order brute force — so the frame-shrink+coloring fix is beyond the permuter's randomization (consistent with the "frame-SIZE caps need manual spill-elimination" note above; the new datum is that reseeding still buys ~0.1-0.2pp/pass of partial fuzzy credit on the cascade).

**The permuter score is NOT objdiff fuzzy.** Always splice each candidate body in-tree and `objdiff-cli report generate -o /tmp/r.json` to read the real `fuzzy_match_percent` before keeping it (a lower permuter score reliably tracked higher fuzzy here, but confirm). `report generate` STOMPS the git-tracked `report.json` — `git checkout HEAD -- report.json` afterward. Clean up `nonmatchings/scratch*` dirs (gitignored, but they pile up).
