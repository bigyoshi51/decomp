...
- FLOORS (don't burn time): (1) **commutative operand-order** — FPU `mul.s`/`add.s` operand swap where a value is pinned by ABI (`$f0` call-return), `gl_func_00052104` 15→5; AND integer `beq`/`bnel` operand-order ($s-first vs $t-first emit normalization), `uso_skip_to_end` 20→10. (2) **register PLACEMENT** — value in `$v1`+`move v0,v1` vs direct `$v0`, `game_libs_func_0002831C` floored ~60. (3) **branch-likely LAYOUT** — `beqzl`-to-end arm placement, `game_libs_func_0005330C` floored ≥885. Rule of thumb: if the 2 diffs are a *reorder of when instructions are emitted*, permute; if they're *which register/operand-slot holds a value* or *which arm is laid out where*, it's a structural cap — leave NM/INCLUDE_ASM.
