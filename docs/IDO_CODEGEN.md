...
- [**Volatile-pad cracks frame SIZE with zero insns, but named locals can't be positioned BELOW a compiler-generated spill (boundary of the pad levers)**](#feedback-ido-pad-cant-go-below-compiler-spill) — _A `volatile double` or `volatile int pad[2]` unused local grows the frame by 8 with NO extra instructions (no load/store emitted), which fixes a too-SMALL-frame near-miss's prologue + arg-home cascade. BUT: IDO lays named locals (incl. the pad) ABOVE compiler-generated spill slots, so the pad takes the high slots and any compiler spill (e.g. an `sw aN,X(sp); lw aN,X(sp)` arg re-materialization) drops to a LOW slot. If the target instead has that spill HIGH with the unused gap BELOW it (unused-low / spill-high), no named pad can reproduce it — verified exhaustively on `gl_func_0000E6E8` (volatile double/int/int[2] all crack frame 0x28→0x30 but leave the a1-spill at 0x24 vs target 0x2C; 2 irreducible diffs). **For a COMPILER-spill-position residual, the pad levers don't apply — instead make the spilled value a NAMED local and use the [decl-permutation brute-force](#feedback-ido-interleave-decl-spill-slot-alignment) (stack offsets ARE decl-order-driven).** Only reach for pads when the mis-positioned slot is itself a named/`&`-taken local._
