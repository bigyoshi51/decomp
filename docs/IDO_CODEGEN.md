...
- [Single-word STRUCT copy emits address-recompute; scalar read emits direct-index](#struct-copy-recompute-vs-scalar-index) — _In a reader-template (`f(&D,buf,N); *dst=…`), a struct copy `*dst=*(One1*)buf` makes IDO re-materialize the buffer address (`addiu rX,sp,off; lw 0(rX)`, regs t7/t9/t6) where a scalar `*dst=buf[0]` emits a direct `lw off(sp)`. Use the struct form when the post-call diff is recompute-vs-direct-index. Byte-exact game_uso_func_0000035C 2026-05-31; debunked a "regalloc split-piece, permuter-only" NM cap — permuter floors (base 235) because it can't change access form._
...
- [`*(volatile int*)&argN` homes a named arg to its caller-arg-slot (`sw argN,K(sp)`); the read still materializes a pointer, not a direct spill-reload](#feedback-ido-volatile-amp-arg-homes-it) — _When a leaf's target homes a register arg to its caller-arg-save slot and your C leaves it in the register, read it once as `*(volatile int*)&argN` to force the `sw argN,K(sp)` home (recovers the home + shifts sibling homes; got 7-insn-exact prologue on game_uso_func_0000D5F8). LIMIT: the read compiles to `addiu rX,sp,K; lw 0(rX)`, never the target's direct `lw K(sp)` spill-reload — that's allocator-only. Varargs is NOT the fix (adds a frame)._
- [Single-return NAMED-LOCAL routes a float result through a callee temp ($f2) → `mov.s f0,f2` tail, vs direct-$f0 from `return expr`](#feedback-ido-named-local-float-via-f2-tail) — _When the target computes a float into a NON-return FP reg ($f2) and ends `… ; jr ra ; mov.s f0,f2` (and the null/early arm does `mtc1 zero,f2`), the `return expr;` / early-`return 0.0f;` forms emit the result DIRECTLY in $f0 (no mov.s). Fix: one `float ret;` assigned in every arm + a single `return ret;` at the end — IDO allocates `ret` to $f2 and moves to $f0 at the unified return, matching the via-$f2 tail. Debunked game_uso_func_00007A98's "C can't reproduce via-$f2 (22 variants)" cap. CAVEAT: a constant-0 return still copy-propagates to $f0 even as a named local — the via-$f2 form needs a genuine computed value in some arm. Related: a tiny no-caller "function" that is another fn's `beql`-past-end tail is a MIS-SPLIT shared epilogue (whole-file cross-jump) — verify callers before treating it as matchable; needs a boundary merge, not a per-fn C body._
...
- [**Remove an "obvious idiomatic local" and recompute inline** — IDO CSE's it AND schedules the spill into the jal delay slot at a HIGHER in-frame offset](#feedback-ido-remove-local-recompute-inline-for-jal-delay-spill) — _When a function spills a small computed value (`char *newA0 = a0 + 0xA0`) across a `jal`, the natural `T name = expr;` local makes IDO -O2 spill `name` to the LOWEST in-frame slot (sp+0x18 in a 0x20 frame), BEFORE the jal. Target has the spill in the jal's DELAY SLOT at sp+0x1C. **Lever: delete the local; recompute the expression inline at every use.** IDO CSE collapses the recompute back to one value AND schedules its spill into the delay slot at the higher slot (0x1C, not 0x18). 9-variant grind (alias, &a0-home, register, volatile, decl-order swap, int pad, static pad, K&R, etc.) all failed before this; removing the local was the simplest possible change. Verified byte-exact 2026-05-27 on `gl_func_0004E180` + `gl_func_0002FA90` (16 + 32 insns respectively). Generalizes for any near-miss whose ONLY residual is "in-frame spill at lower slot than target wants" — try the no-local lever before declaring an allocator cap. **FAILURE MODE (verified 2026-05-28 on func_00007C74 + game_uso_func_00000724): only works when the value has essentially ONE consuming use across the jal so CSE collapses the recompute to a single spill. When the local has MANY uses — e.g. a loop-condition read + after-loop reads, or 4 derefs bracketing a call — inlining makes IDO emit a fresh reload at each use site and REGRESSES (func_00007C74 6→19 diffs, game_uso_func_00000724 ~5→18 diffs). Check the local's use-count first; if it's used >1-2 times across calls, skip this lever.** **EXTENSION (2026-05-31, `game_uso_func_0000F284`, byte-exact): the lever also fires when you remove a redundant ARG-ALIAS local (`int *s0 = a0;` → use `a0` directly) and the spilled value itself is branch-merged / NON-recomputable. Here `m` (= `cond ? 0x20008 : a0[0x3F]|8`, spilled across a middle `jal`) wanted sp+44 but the build emitted sp+40 in a 0x30 frame. `m` can't be inlined (branch-dependent value used after the call), but deleting the idiomatic `s0=a0` alias and dereferencing `a0` everywhere shifted IDO's pseudo-numbering enough to reorder the spill-slot assignment and bump `m` to the higher slot (sp+44) → byte-exact. TAKEAWAY: for a spill-slot-PLACEMENT residual, the real mechanism is the pseudo-RENUMBER, not the CSE — so try removing ANY redundant named local (a pure arg alias is the cheapest), not only the spilled value's own local. Invalidates "spill-slot placement not C-controllable" caps even when the spilled value is non-recomputable.**_
...
- [Auto-unroll "by-N element vs by-1 trip counter" is NOT a cap — NESTED loop + comma-`for` cracks it](#feedback-ido-unroll-element-vs-trip-counter) — _**CRACKED 2026-05-30 — game_libs_func_0005BDC0 LANDED byte-exact (was documented a "genuine cap" for months).** The mistake was using a FLAT count loop: `for(i=0;i<16;i++) dst[i]=1/src[i]` 4x-unrolls to a by-4 ELEMENT counter (cmp 16); a literal 4-trip loop fully unrolls. **The fix is a NESTED N×M loop where the inner loop unrolls and the outer stays a trip counter:** `for(i=0;i<4;i++){ for(j=0;j<4;j++) *d++ = 1.0f/(*s++); }` — the inner 4-loop fully unrolls (pointer pre-advance + negative offsets), the OUTER stays a 4-trip ROLLED loop with the target's exact by-1 trip counter (cmp 4). That alone gets the structure + a 3-insn prologue-schedule residual. **Kill the last 3 (the setup-move ordering `bound,counter,dst`) with the comma-`for` lever** (same crack as the gl_func_0005C6C4 entry below): `for (i = 0, d = dst, s = src; i < 4; i++)` — the comma-inits IN THAT ORDER (counter, dst, src) schedule the 3 independent moves exactly like the target. Reordering the comma-inits (e.g. `i,s,d`) → 12 diffs; the right order → 0. So: nested loop for the IV form + comma-`for` (counter first) for the prologue schedule. The old note (kept below for archaeology) wrongly concluded "allocator-internal, keep the element-counter form" after trying only flat forms + plain decl-order. **[SUPERSEDED]:** ~~flat `i<16` gives a by-4 counter; goto-loop form floors at a 3-insn permutation; IDO scheduler robust against decl-order/register/volatile; genuine cap, keep element-counter ~99.9%.~~ — the comma-`for` IS the decl-order lever that flat-form attempts never reached._
...
- [Short-return stored AND tested: keep `a` as the raw `int` return — store `(short)a` (emits `sh v0`, raw) but test `(short)a` (emits `sll/sra`-then-branch on the sign-extended temp)](#feedback-ido-short-return-store-raw-test-sext) — _When a target stores a callee's return as a halfword (`sh v0, OFF(base)`) AND branches on its sign-extended value (`sll t,v0,0x10; sra t2,t,0x10; bne t2,zero`), declare the local as `int a = call();` (NOT `short a = call();` / `a = (short)call();`). Then `*(short*)p = (short)a;` stores straight from v0 (the store only writes low 16, so no sign-extend needed → `sh v0`), while `if ((short)a == 0)` forces the separate `sll/sra` sign-extend for the COMPARE. Writing `a = (short)call()` puts the sign-extended value in a temp reg (v1) and stores THAT (`sh v1`) — a register diff. Verified gl_func_0002BAAC 2026-05-31. Pairs with the unconditional-store-in-bne-delay-slot idiom (store sits in the branch's delay slot so it runs regardless of the condition)._
...
- [do-while whose target shows BOTH `beql`(entry) AND `bnel`(loop-back) = peeled first iteration — rewrite `do{x}while(c)` as `x; while(c){x}`](#feedback-ido-peeled-first-iter-while) — _When a `do { v = call(); } while (v != K);` loop's TARGET has two branch-likelies — a `beql v,K,exit` right after the FIRST call (entry guard) AND a `bnel v,K,loop` at the bottom (loop-back) — IDO peeled the first iteration. The plain `do-while` C emits only ONE `bnel` (loop-back), N insns short. Rewrite as the explicit peeled form: `v = call(); while (v != K) { v = call(); }` — the call appears twice in source; IDO emits `call; beql v,K,end; loop: call; bnel v,K,loop; end:` matching exactly. Cracked gl_func_0006612C 64.7%->94.8% (+30pp, 2026-05-31) on two magic-constant spin-wait loops. Recognition: count the loop's branch-likelies in the target — TWO (beql+bnel) per logical loop = peel it; ONE (bnel) = plain do-while. (Distinct from the unroll-defeat lever below, which is about rolled-vs-unrolled bodies.) Residual on 6612C was prologue save-order only._
...
<a id="feedback-ido-volatile-amp-arg-homes-it"></a>
## `*(volatile int*)&argN` homes a named arg to its caller-arg-slot (`sw argN,K(sp)`); but the read recompiles as a pointer-materialize, not a direct spill-reload

When the target homes a register arg to its caller-arg-save slot (`sw a3,0xC(sp)` — no frame, leaf) and the natural C leaves that arg purely in its register, read the arg once as **`*(volatile int*)&argN`**. The `&argN` makes the arg addressable (IDO emits the `sw argN,K(sp)` home, often in a nearby branch-delay slot) and `volatile` forces the value from memory. This recovers the home + shifts the other arg-homes to match (verified game_uso_func_0000D5F8 2026-05-31: first 7 insns became byte-exact, debunking a "can't force a3 home" note).

**Limit — the read itself can't be a *direct* spill-reload.** `*(volatile int*)&argN` compiles to `addiu rX,sp,K; lw 0(rX)` (pointer-materialize), whereas a target that spill-reloads the arg uses a direct `lw K(sp)` (one insn fewer). The C forms of reading a homed arg are: **register** (named `argN` / non-volatile `(&a1)[2]` — copy-propagates back to the arg reg) or **pointer-materialize** (any `volatile`/`&`-deref → `addiu+lw`). The allocator-driven *direct* spill-reload (`lw K(sp)`) is NOT reachable from a C leaf — if the only residual is that one `addiu`, it's a genuine varargs-home + direct-spill-reload artifact. (True varargs `(char*,int,...)` is NOT the fix — it adds a stack frame + `v0/v1/bnel`, diverging entirely.)

<a id="feedback-ido-named-local-float-via-f2-tail"></a>
## Single-return named-local routes a float through a callee temp ($f2 → `mov.s f0,f2`); `return expr` goes direct-$f0

When a near-miss's only structural difference is that the target keeps a float result in a non-return FP reg and ends with an explicit move to $f0 — e.g.
```
sub.s f2, f4, f6 ; jr ra ; mov.s f0, f2      ; non-null path
mtc1 zero, f2    ; ... ; jr ra ; mov.s f0, f2 ; null path
```
the natural `return v1->bc - v0->bc;` (and early `return 0.0f;`) emit the result **directly in $f0** with no `mov.s`. Coalesce to a **single return through a named local**:
```c
float ret;
if (v1 == 0) ret = 0.0f;
else         ret = *(float*)((char*)v1+0xBC) - *(float*)((char*)v0+0xBC);
return ret;
```
IDO -O2 allocates `ret` to $f2 and emits `mov.s f0,f2` at the unified epilogue, reproducing the via-$f2 tail in every arm. Debunked game_uso_func_00007A98's documented "C can't reproduce the via-$f2 form (22+ variants)" cap (2026-05-31).

**Caveat 1 — constant-0 stays $f0.** A pure `return 0.0f`/`float r=0; return r` copy-propagates to `mtc1 zero,$f0` even as a named local (probed named-local/volatile/int-store). The via-$f2 form needs a genuinely *computed* value in at least one arm; an all-constant function can't be forced to $f2 in isolation.

**Caveat 2 — no-caller tail = mis-split, not a function.** game_uso_func_00007ABC (`mtc1 zero,f2; jr ra; mov.s f0,f2`, returns 0.0f) had **no independent callers** — it is 7A98's `beql`-past-end shared epilogue that splat labeled as its own function. The full 9-insn match needs IDO's whole-file cross-jump to merge 7A98's null tail into it (which needs 7ABC *also* in $f2 — blocked by Caveat 1). The real fix is a boundary MERGE, not a per-function C body. **Always check callers (`jal`/reloc/symname) of a tiny constant-returning "function" that sits right after a `beql`-past-end parent before trying to match it standalone.**

<a id="struct-copy-recompute-vs-scalar-index"></a>
## Single-word STRUCT copy emits address-recompute (`addiu rX,sp,off; lw 0(rX)`); scalar read emits direct-index (`lw rX,off(sp)`)

In the USO reader-template family (`f(&D, buf, N); *dst = <result>;`), the post-call read of the stack buffer takes one of two forms depending on whether the C reads a **scalar** or copies a **struct** — even for a single word:

- **Scalar** `*dst = buf[0];` → IDO knows `buf` is at a fixed sp offset, emits a **direct-index** load: `lw t6, 0x18(sp); lw t7, 0x20(sp); sw t6,0(t7)` (3 insns). This is what `eddproc_uso_func_00000000` matches.
- **Struct copy** `*dst = *(One1*)buf;` (One1 = `struct{int a;}`) → IDO **re-materializes** the buffer address into a register and walks it with offset-0 loads: `addiu t7,sp,0x18; lw t9,0(t7); lw t6,0x20(sp); sw t9,0(t6)` (4 insns, registers t7/t9/t6, skipping t8). This is the multi-word struct-copy codegen (`game_uso_func_0000039C`'s Quad4 walk) applied to one word.

So when a reader-template near-miss shows the **address-recompute** post-call form (`addiu rX,sp,off` then `lw 0(rX)`) instead of a direct `lw off(sp)`, the source copies the result as a STRUCT, not a scalar. Keep the buffer sized to its real footprint (`int buf[2]` → 8 bytes at sp+0x18, frame 0x20) and copy only the consumed words out as a struct: `*dst = *(One1*)buf;`. Byte-exact `game_uso_func_0000035C` 2026-05-31 — this debunked a documented "regalloc split-piece coloring, permuter-only NM cap" (the t7/t9/t6-vs-t6/t7/t8 note); it was never a coloring problem, just scalar-vs-struct access form. The permuter floors on it (base 235) because its mutations don't change the access *form*. Try this before the permuter on any reader-template residual whose only diff is direct-index vs address-recompute.
...
**To MATCH a cluster-load (target = `lui;addiu base; lw 0(base); lw 4(base)`), the address-form is NOT the blocker — register PRESSURE is** (verified 2026-05-30 on `game_uso_func_0000FC34`). A distinct-extern base (`extern int D_00000E10; int *p = &D_00000E10; p[0]; p[1]`) emits the exact target `lui+addiu`-base-then-`lw 0/4` form **STANDALONE**, but in a high-pressure full function IDO **re-materializes the base per access** (two `lui`s) anyway — the named base can't stay live. The +-form vs fold-form is a red herring under pressure; switching to the distinct extern moved fuzzy +0.04% only. So before chasing a cluster-load (or a `field+N` base-pointer fold like `lwc1 1920(v1)` vs `addiu a3,v1,1904; lwc1 16(a3)`), check whether the function is register-bound — if it spills/re-materializes, the cluster won't hold until the *function-wide* pressure is reduced (fewer simultaneously-live locals; a regalloc-dump session), regardless of how the base address is written.

**At LOW pressure the distinct-extern DOES hold in-tree** (verified 2026-05-30 `game_uso_func_0000D7F4`): a function that already keeps the cluster base in one register in-tree (not re-materializing) DOES adopt the target `addiu base,0xOFFSET; lw 0(base); lw 4(base)` form when you switch `&D_00000000 + 0xOFFSET` → a distinct `&D_0000OFFSET` extern (reloc-verified). So the distinct-extern is the right fix for cluster *form* whenever the function isn't register-bound; it's only futile under the high pressure that forces per-access re-materialization (FC34). The base *register* (e.g. `$v0` vs target `$t9`) may still differ — that's a separate caller-saved-temp renumber, orthogonal to the address form.
...
**STALE-CAP CATCH (2026-05-31).** `game_uso_func_00011124` landed byte-exact by simply UNWRAPPING it — its body was ALREADY `*(Pair2*)(&D+0xF50)` struct-by-value (so it compiled to the exact bytes: single `$t6` base + `sw a1,4(sp)`/`sw a2,8(sp)` homes) but it was still `#ifdef NON_MATCHING`-wrapped behind a stale "82.18% pre-call-spill cap" comment whose variants all predated this lever. **Meta-lesson: a pair-passing function whose body is already struct-by-value but still carries a pre-2026-05-28 "pre-call-spill"/"NATURAL CEILING" comment may ALREADY be byte-exact — byte-verify the build .o against expected/.o (`.text` only) and unwrap, don't re-grind.** (A374 is NOT one of these — it passes single args, no adjacent pair, genuine 72-vs-80-byte near-miss.)

**EXTENSION — a SINGLE int arg, to home a "dead" outgoing arg (2026-05-31).** struct-by-value also homes ONE int. When a 2-arg call `f(0, a0)` shows the target redundantly homing the 2nd arg to its outgoing slot (`sw a1,4(sp)` — dead, the callee never reads it) and your natural call emits a register move (or a 3rd-arg copy `or a2,a1`), pass the int as a single-field struct: `f(0, *(struct OneI*)&a0)` (OneI = `struct{int x;}`). IDO homes it to `4(sp)` exactly. Cracked the bootup `func_00005068`/`func_000054A0` "3rd-arg-copy-vs-dead-home cap" (whose prior notes tried `(void)a1`, 4 variadic decls, `&param`, fn-ptr cast, and the permuter — but never struct-by-value). So treat ANY "dead outgoing-arg home" / "3rd-arg-copy" residual as a struct-by-value candidate, single int included.

**EXTENSION — CONDITIONALLY-SELECTED pair: duplicate the call in each arm + swap arms for branch-likely polarity (2026-05-31).** When the pair to pass is chosen by a condition — `if (cond) f(x, D[A], 1) else f(x, D[B], 1)` — three moves together reach byte-exact (cracked `game_uso_func_00011624`, 70.6%→100%): (1) pass each pair as `*(Pair2*)(&D+off)` by value; (2) **DUPLICATE the whole call inside each if-arm** rather than selecting a pointer/value and calling once — a select-then-call-once builds a redundant local struct copy on the stack (`addiu v0,sp,N; sw…0(v0); sw…4(v0); lw a1,0(v0)`) and loses the branch-likely; the target instead precomputes BOTH `&D` base pointers, then loads the chosen pair's words directly from the base in each branch; (3) **try both arm orders** — the target uses a branch-LIKELY (`bgtzl`/`blezl`) whose taken-arm's first D-pair load sits in the delay slot, so the arm written second vs first flips `bgtzl`↔`blezl` and the two base-pointer register assignments; pick the order that matches (here `<= 0` first gave `bgtzl` + correct `t0=&D+lo / t3=&D+hi` order). All cost-free to A/B. So: a "conditional D-pair / beql-selected mode+D-pair" cap is a struct-by-value candidate, but you must duplicate the call per-arm and A/B the arm order.
...
**Stolen-prologue base-load variant (verified 2026-05-30, timproc_uso_b5_func_0000E5D8 — first 100% of the stolen-prologue boundary-fix vein):** a "prologue-stolen successor" whose pre-prologue loads a GLOBAL POINTER (`lui v0; lw v0,0(v0)` = `*(&D)`, a 2-insn compact global read) and ALSO uses `&D` for store destinations in its body. Two fixes compose: (1) merge the 2 stolen-prologue words from the `_pad.s` sidecar into the function's own `.s` (boundary correction, same as gl_func_0001FC78/000601DC); (2) the base read must be the compact 2-insn `lui v0; lw v0,0(v0)`, but with a single `&D_00000000` IDO CSEs the `&D` address between the base read and the dst stores → it materializes `lui v1; addiu v1,0` once and emits the base load as 3-insn `lw v0,0(v1)`. Bust that with a DISTINCT extern read directly as a value: `extern int D_x_base; int *base = (int*)D_x_base;` (alias `D_x_base = 0x0`) → the base read becomes the compact `lui v0; lw v0,0(v0)` and the dst keeps its own `&D` materialization, matching. KEY DISTINCTION from the register-renumber stolen-prologue caps (FC78/601DC stay NM): those load into `$t6` (renumber, not C-forceable); E5D8 loads into `$v0` (IDO's natural first-temp), so once the CSE is busted it matches 100%. **When triaging a stolen-prologue successor: if the pre-prologue register is `$v0`, it's likely matchable (merge + CSE-bust); if it's `$t6`/other, expect a register-renumber residual.**
...
**SCOPE LIMIT (verified 2026-05-30, `titproc_uso_func_000000C0` REGRESSED 97.0%->83.7% by over-applying):** in-place-RMW only wins when the TARGET re-reads the field fresh at *each* use. If the target CACHES the counter at some points (reuses one register across several uses), the in-place rewrite emits an EXTRA `lw` at every use and regresses badly. Before converting, check the target disasm: count its `lw …,OFF(base)` of the counter — if it re-reads at every use (like 16B8's per-branch reads), in-place matches; if it loads once and reuses (some `if(cnt==N)` chains keep the value live), KEEP the named local. The lever is for fresh-read-per-use shapes, not "any field counter".
...
**A `Tried: -O0 (N%), -O1 (M%), -O2-g3 (K%) wins` table inside an NM-wrap comment is a record of the C SHAPE that was tried at each tier, NOT proof that the tier is wrong.** When a later pass shows the target is clearly -O0, do not let the recorded `-O0 %` talk you out of re-testing -O0 with a *cleaner* C body.

**Case: bootup_uso func_00012188** (47-insn struct initializer, sat at 55.30% NM for weeks). The NM comment carried:
...
That `-O0 (35%)` was measured against a contorted body: `volatile s32 sp4; do { ...; next = sp4+1; sp4 = next; } while (next < 3);` plus `v = 1; *(..)=v;` temp-var stores. The `volatile sp4` and do/while fought the -O0 allocator. The actual target is plain -O0:

- early `=1` stores each get a FRESH temp (`addiu $t6,zero,1` / `$t7` / `$t8` / `$t9`) → **direct constant assignment** `*(s32*)(a0+0x4C) = 1;` (NOT a reused `v`)
- 3-byte init loop reads/writes its counter from `4(sp)` every iteration → plain `for (i=0;i<3;i++)` (at -O0 `i` lives on the stack automatically; **no `volatile` needed**)
- the later value-PAIR stores reuse one register `$a1` → `register int v; v = 4; *(..)=v; *(..)=v;`
- unfilled-delay epilogue `b L; nop; addiu sp,8; jr ra; nop`

The clean body is byte-exact at -O0 (47/47 words). The recorded table had simply never tried direct-constants + bare `for` at -O0.
...
**PERMUTER-CONFIRMED 2026-05-30**: ran the decomp-permuter on titproc_uso_func_00000418's `lw $5,0x18(sp)` <-> `or $2,$0,$0` swap for **97,000 iterations** — its best real `.text` diff never dropped below the 2-insn swap (it found a `score 15` variant via an `if(selected){}` dummy uninit-read, but that only lowers the permuter's internal penalty; the swapped pair is unchanged in the emitted bytes). The IDO scheduler's tie-break between two ready independent instructions is NOT reachable by any C mutation the permuter explores — this cap class is permuter-proof, not merely C-hard. Do NOT stand up a permuter run on a pure 2-insn scheduling swap. (Setup if needed for OTHER cases: `tools/permuter/import.py <src.c> <func.s> RUN_CC_CHECK=0` then `permuter.py nonmatchings/<func> -jN --stop-on-zero`; Makefile `PERMUTER=1` mode is wired; use the ABSOLUTE monorepo path for import.py — the worktree `tools/` symlink doesn't resolve.)
...
## "Caller-set float-in-$fN convention cap" is often a STOLEN-PROLOGUE ORPHAN — check for an adjacent no-jr-ra stub that loads the constant

When a USO function reads `$f4` (or any non-`$f12` float reg) via `swc1 $f4, OFF(a0)` WITHOUT setting it, the easy conclusion is "caller passes a float in `$f4`, a non-O32 convention IDO can't express." **Verify that's true before accepting the cap.** Look at the immediately-PRECEDING symbol: if it is a tiny stub with NO prologue and NO `jr ra` (e.g. `lui at,0x3F80; mtc1 at,$f4` = load 1.0f), the two are ONE function that splat split at the body's prologue. The stub loads the constant into `$fN`; IDO **hoists that const-load above the prologue**, so it lands at the stub's address and falls through into the body. `$fN` is not a caller convention — it's the function's own constant.
...
**Generalizes beyond float consts.** The pre-prologue hoist works for ANY orphan whose body reads the loaded register first — including an INTEGER global load `lui rN,0; lw rN,off(rN)` (a flag/pointer). IDO schedules the load above the prologue when the body opens by using rN (e.g. `bne t6,...`). Two finishing tips that recur: (a) access the global as `*(T*)((char*)&D + off)`, NOT a `D_..._alias` symbol — the alias folds the offset into the reloc (`lw 0(rN)`) whereas the target keeps it in the load (`lw off(rN)`); (b) if the body branches and a pointer like `a0->0xB4` is used in BOTH arms, RECOMPUTE it inside each arm rather than hoisting one shared local — that flips IDO from a shared `$v0` to the target's per-arm `$t7`/`$t2` temp allocation. Took `game_uso_func_0001155C` byte-exact (32 insns: 2-insn flag orphan + 30-insn dispatch body the prior pass had stuck at 81.93%). Verified 2026-05-30.
...
## Hardware-register (SP/VI/etc) accessors with a LEADING NOP are not C-matchable — leave INCLUDE_ASM

Tiny memory-mapped-I/O accessors of the shape `nop; lui rN,0xA4xx; jr ra; lw/sw rN,off(rN)` (e.g. 1080 game_libs `00074840`/`00074850` = RSP SP_STATUS 0xA4040010 write/read, `00069F50` = VI 0xA4400010 read) carry a **leading `nop`** before the `lui` — a pipeline-hazard idiom for the hardware touch. Plain C `*(volatile int*)0xA40400xx = a0;` / `return *(volatile int*)0xA40400xx;` emits the right `lui;jr;sw/lw` (the load/store correctly fills the `jr` delay slot) but WITHOUT the leading nop, so it never byte-matches. IDO -O2 has no way to inject a bare nop from C: `__asm__("nop")` is parsed as a JAL to an external symbol named `nop` (not an emitted instruction), and `__asm__ volatile(...)` is a cfe syntax error (see feedback_ido_no_gcc_register_asm). Recognize the `lui 0xA4xx` + leading-nop shape and leave it INCLUDE_ASM — don't grind it. (The accessor's PURPOSE is still worth documenting inline: decode the 0xA4xxxxxx address via references/indexes/hw_registers.h.) Confirmed 2026-05-30.

## A stray `cvt.d.s` in a float function = K&R float→double arg promotion; fix with a float-prototyped alias

If a float-math function comes out +1 instruction over target with an extra `cvt.d.s` (single→double convert) right before a `jal`, the cause is C default argument promotion: a `float` passed to a function with NO prototype (K&R `extern float f();`) is promoted to `double` at the call, so IDO emits `cvt.d.s $f12,$fN` before the call. The target's original prototyped the callee as taking `float`, so no promotion.

You usually CAN'T just add `(float)` to the global declaration, because these USO placeholder callees (`gl_func_00000000` and its typed aliases) are deliberately K&R — the SAME symbol is called with different signatures elsewhere (e.g. 3 pointers). Instead add a **float-prototyped alias** mapped to the same address: declare `extern float gl_func_00000000_ff(float);`, add `gl_func_00000000_ff = 0x00000000;` to `undefined_syms_auto.txt` (it's a runtime-relocated placeholder, so 0x0 like the base), and call `_ff(...)` only at the float call site. The promotion vanishes and — bonus — the pre-call `add.s`/mul result then lands directly in `$f12` (the arg reg), often fixing a cascade of float-register diffs too. Took game_uso_func_00003ED4 92.5%→99.44% (size-exact 54; residual was an unrelated $f2/$f12 div-result regalloc swap). This is a CLEAN size-mismatch lever distinct from the regalloc caps — scan +1 float functions for a stray `cvt.d.s`. Confirmed 2026-05-30.

## A "spill-not-saved-reg / frame-too-small" near-miss is NOT a cap — split the value into two webs, then permute reg/slot

When a non-leaf's ONLY residual is "target spills cross-call temporaries to stack (no $s saves) with a BIGGER frame than your C produces" (e.g. target frame 0x28, yours 0x20), it is a **register-allocation difference, not a cap**. The recipe that cracked eddproc_uso_func_000003BC (a documented "~94% frame-spill cap") 89%→**byte-exact** 2026-05-30:

1. **Diagnose with the regalloc dump** (`-Wo,-zdbug:6`, the ecvt-patched cc → `uoptlist`): if `numcoloredlr=0`, NOTHING is in $s — both you and the target spill everything, so the frame-size delta is purely the NUMBER of distinct spill slots. A bigger target frame means the target spills ONE logical value as **two separate live ranges** (two slots) where your C coalesces it into one.
2. **Split the value into two C variables** across the branch/merge that separates its two use-regions (`p1` for block1, `p2 = p1` for block2, use/return `p2`). This forces the second spill slot → matches the frame size. (Got the frame exact but the value still in the wrong reg.)
3. **Permuter from the split seed.** The split is a far better seed than baseline (eddproc went base-score 275 → 25). The winning mutation was non-obvious: **reuse the now-dead first variable to hold a later value** (`p1 = arg0;` after `p2 = p1;`) — that freed IDO to place the pointer in `$a0`(block1)/`$v1`(block2) instead of `$a2`, eliminating an extra `move a2,v0` in the branch delay slot.
4. **Brute-force declaration order + condition form** for the last 4-6 diffs (all spill-slot offsets / a temp reg): int spill slots are decl-order-driven (see interleave-decl-spill-slot), and **re-loading a condition** (`if((int*)p1[0x40/4]!=0)` instead of testing the saved `head`) places that temp in `$v0` + a plain `beqz` (vs `$a1`+`beqzl`). 6 decl-orders × {test-saved, re-load} found the byte-exact combo.

Method summary: full asm trace → regalloc dump (confirm all-spilled) → variable split (frame) → permuter (register class) → decl-order + condition-reload brute-force (slots + last temp). The permuter ALONE floored at 275 (it can't invent the structural split); the split ALONE got the frame but not the regs; together → 0. Do NOT label this class "cap" — it is mechanically reproducible.

## Strength-reduce-multiply vs hand-expanded chain — let IDO do the `x * N`

**Symptom:** target computes a constant-stride array index as a multiply chain that
reuses a SINGLE register throughout, e.g. `a0 * 100`:

```
sll   t7, a0, 2     # a0*4   (often HOISTED above the prologue — orphan/stolen-prologue)
subu  t7, t7, a0    # a0*3
sll   t7, t7, 3     # a0*24
addu  t7, t7, a0    # a0*25
sll   t7, t7, 2     # a0*100
...
Every step writes back to `$t7`. When you hand-expand the arithmetic in C to try to
force the hoist —

```c
int t = a0 << 2; t -= a0; t <<= 3; t += a0; t <<= 2;   /* WRONG */
```

— IDO treats each sub-expression result as a fresh pseudo and the register allocator
spreads them across `$t8/$t9/$t0/$t1` (mnemonics match, dest registers don't → ~96.8%
fuzzy that *looks* like a regalloc/"hoist-breaks-reuse" cap but is self-inflicted).

**Fix:** write the index as the plain multiply and let IDO's strength reducer expand it:

```c
gl_ref_00045DC0(&gl_ref_00000368 + a0 * 100, 1, a1, 0x7F);   /* RIGHT — 100% */
```

IDO's own strength reduction emits the exact `sll;subu;sll;addu;sll` chain on one
register and hoists the first step above the prologue. **Try the literal `x * N`
BEFORE hand-expanding or declaring a strength-reduction chain a register cap.** Verified
2026-05-31 on `game_libs_func_000315BC` (was documented as a hoist-breaks-reuse cap in
...
<a id="branch-likely-emission-cap-no-flag"></a>
