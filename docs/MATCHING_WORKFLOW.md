...
The remaining MATCHABLE work needs per-file opt carves, which for post.c means a mid-file split (post.c is one sequential .o in tenshoe.ld at VRAM 0; carves only place correctly at a point in the .o sequence). post.c holds 1102 functions, mostly address-ascending (14 out-of-order pairs to watch). To carve function F: split into post.c [funcs before F] + game_libs_{o1,g3}_<addr>.c [F, with OPT_FLAGS + TRUNCATE_TEXT like game_libs_o0_8A40.c] + game_libs_post2.c [funcs after F], list all three in sequence in tenshoe.ld, and refresh-expected-baseline for all three. Top targets: gl_func_00070244 (CRC-8 poly 0x85, exact C already in its comment, matches at -O1 — index 1011/1102) and the unfilled-delay cohort (each matches at -O2 -g3). This is focused-session work, not a 60s tick.
