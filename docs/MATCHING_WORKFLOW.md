...
**Scale limit (2026-05-08):** the sibling-port test only works across files of similar size+complexity. The SAME C body that lands byte-exact in small files (50-200 functions: titproc_uso.c, mgrproc_uso.c, arcproc_uso_tail1.c) can REGRESS in large files. Verified on `gl_func_000088B4` (in game_libs.c, ~1500 functions): porting the matched titproc 2980 / mgrproc 33358 / arcproc 2334 form dropped fuzzy 89.31% → 77.83%. Under game_libs.c's allocator state, the SAME C source produced different if-branch direction (bne+goto-init vs target's beq early-exit) AND different frame size (-0x20 vs -0x28). Cause: file-level context (compilation-unit-wide allocator state, prior functions' codegen perturbing IDO's state) diverges enough that the body's emit shifts.

**Practical filter:** before applying the sibling-port test, check whether the source AND target functions live in files of similar size. If the matched sibling is in a 50-function file and your target is in a 1500-function file, expect the port to fail. The recipe still saves time on intra-file ports (where allocator state is shared) and across files of similar size; just don't trust it across the order-of-magnitude size gap.
