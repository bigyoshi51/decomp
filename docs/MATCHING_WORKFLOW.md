...
- [For a USO data reference, use `&D_00000000 + 0xNNNN` (addend rides the reloc) — NOT a separate `gl_ref_0000NNNN` extern (leaves `lui 0`, won't objdiff-match)](#feedback-data-ref-addend-idiom-vs-separate-extern) — _Raw-word USO .s bakes data refs as concrete `lui/addiu`. `(char*)&D_00000000 + 0xNNNN` makes asm-processor compute %hi/%lo at assemble-time (correct lui byte + reloc-with-addend → objdiff matches); a separate undefined `gl_ref_XXXX` extern leaves `lui 0` (deferred to link reloc) and silently caps the function. undefined_syms_auto.txt is link-time-only, doesn't help. Verified +pp on gl_func_00000A8C 2026-05-28._
...
## For a USO data reference, use `&D_00000000 + 0xNNNN` (addend rides the reloc) — NOT a separate `gl_ref_0000NNNN` extern (leaves `lui 0`, won't objdiff-match)

_A raw-word USO `.s` bakes data refs as concrete bytes (e.g. `lui a1,0x1; addiu a1,a1,0xCB84` for address 0xCB84), no reloc. To reproduce that from C and have objdiff match it, write `(char*)&D_00000000 + 0xCB84`: asm-processor computes `%hi/%lo(0 + 0xCB84)` at ASSEMBLE time, baking the correct `lui 0x1` immediate into the .o byte (plus an R_MIPS_HI16/LO16 reloc to D_00000000 carrying the addend) — objdiff's reloc-aware compare then matches it against the raw-word baseline. A SEPARATE `extern char gl_ref_0000CB84;` does NOT work: the symbol is undefined in the .o, so the assembler can't compute `%hi` and leaves `lui 0x0` (deferring to a link-time reloc) → the lui byte (0x0 vs 0x1) mismatches and the function silently caps below 100 %._

**Verified 2026-05-28 on `gl_func_00000A8C`:** switching the two data refs from `&gl_ref_0000CB84` / `&gl_ref_0000CB8C` externs to `(char*)&D_00000000 + 0xCB84` / `+ 0xCB8C` moved both lui+addiu pairs from mismatching to matching. (Adding `gl_ref_0000CB84 = 0x0000CB84;` to `undefined_syms_auto.txt` did NOT help — that file is link-time-only and never touches the pre-link .o bytes objdiff reads; see [[feedback-undefined-syms-link-time-only-doesnt-fix-o-jal-bytes]].) The sign-extended `%lo` (here `addiu` of a negative imm) and the `lui` carry are handled automatically by the assembler — don't hand-compute the split. Sibling NM wraps that still use separate `gl_ref_XXXX` externs (e.g. `gl_func_000001CC` with `gl_ref_0000CAFC`) are capped for exactly this reason; convert them to the addend idiom.
