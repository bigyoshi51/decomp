...
2. 3-way source split: parent keeps funcs before F; new `<seg>_g3_<addr>.c` = F alone (+ `#include "common.h"` + any externs/typedefs/macros F uses); new `<seg>2.c` = funcs after F (+ the same header deps — scan the moved range for GL_* macros / typedefs / structs and copy their decls).
