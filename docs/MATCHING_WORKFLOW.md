...
- **NM clip hard-errors the BUILD when a body SHRINKS below the clip value (tail 0x5544->0x5530, 2026-07-15)** — _The growth direction mis-scores (62F08); the SHRINK direction fails the NM build outright: `.text 0xNNN smaller than requested` → `make non_matching_objects` Error 1, which blocks the land script mid-run (its output dies among inject-suffix lines — grep the land log for `make: ***`). A redecode that REMOVES instructions (e.g. retyping away swl/swr pairs) shrinks the unit. FIX: re-measure the unit clip DOWN to the new natural size (keep per-fn resizes), force-delete the .o, rebuild, exact-set diff._

- **A killed/timed-out land or baseline-refresh leaves the WHOLE TREE in the INCLUDE_ASM-swapped state (2026-07-15)** — _refresh-expected-baseline.py (also invoked inside land-successful-decomp.sh) swaps every decomp'd body to INCLUDE_ASM for the baseline build and restores in a finally-block. A SIGTERM/timeout mid-swap (e.g. a 10-minute Bash timeout on a multi-function land loop) kills it before the restore: 100+ source files left stripped (massive deletions in `git diff`). All content is committed beforehand, so recovery is just `git checkout -- .` + rebuild + `cmp` verify. TELL: land script refuses with "current worktree has tracked changes" and `git diff --stat` shows huge deletions across src/. PREVENTION: land in smaller batches (≤5 fns/command) or raise the Bash timeout; never Ctrl-C a land mid-run without checking `git status` after._

- **PAD-.s DELETION trap: a 1-word `_pad.s` GLOBAL_ASM block occupies 8 BYTES in the object (asm-processor min-2-word dummy) — deleting it leaves a 4-byte hole and shifts the whole unit (island batch-5, 2026-07-10)** — _When retiring a `_pad.s` sidecar during a promotion/donor wiring, the genuine inter-function alignment word must come back via `SUFFIX_BYTES_FORCE=<fn>=0x00000000` (the 6EF08 precedent). AND: Makefile-variable changes don't dirty the .o — force-delete the affected objects before the verify build or you'll validate a stale layout._
- **A landed-EXACT function can flap to <100 in incremental report refreshes when a SIBLING in the same unit gets a decode-rise — stale expected/*.o (70C44, 2026-07-11)** — _Decode-correctness rise landings (cherry-pick source + refresh report.json) do NOT run `refresh-expected-baseline.py`. When a decode rise lands on function B, the unit's `build/non_matching/<unit>.c.o` is rebuilt but `expected/<unit>.c.o` stays as captured at the last baseline refresh. A previously-landed EXACT sibling A in the same unit can then read <100 in an INCREMENTAL `refresh-report.sh` (its build .o is fresh, expected .o is stale → mismatch), inflating/deflating the tracked count nondeterministically (gl_func_00070C44 flapped 100↔99.53 across agents; the committed report.json was stale-100 while clean rebuilds gave 99.53). It is NOT a broken match — the ROM stays byte-identical and a full `rm -rf build/non_matching && make non_matching_objects` + `refresh-expected-baseline.py` restores the true 100. FIX/HYGIENE: after a batch of decode-rise lands, run `refresh-expected-baseline.py` + a clean NM rebuild to get the authoritative deterministic count before trusting/committing report.json; don't chase a single flapping sibling as a regression. The exact-set-diff `lost:` check will surface these — verify with a clean rebuild before acting._
- **A decode-correctness edit that adds FILE-SCOPE externs/symbols can silently regress a SIBLING compiled-C function in the same .c (timproc_b5 CEB4 broke 1658, 2026-07-10)** — _Reconstructing one function often means adding `extern` decls or `undefined_syms_auto.txt` entries for its callees/globals (e.g. `timproc_uso_b5_func_00004068`, `D_807FFB08`). If another COMPILED-C function in the same file already references those symbols, the new/changed declaration alters ITS codegen too — CEB4's rebuild broke a genuinely-100% sibling (func_00001658) to 14 word-diffs. An agent measuring only the target function in isolation reports "no regression" and misses it. MANDATORY at land time: diff the exact-fn SET (fuzzy==100 names) between origin/main's report.json and the refreshed one — a `lost:` entry is a regressed sibling. If found, the decode edit must be reworked to not touch shared file-scope symbols (or dropped — a sub-100 decode rise is never worth dropping an exact match). The landing pipeline's set-diff catches this; the per-function fuzzy measurement does not._
- **`game_libs_post0b.c` has NEGATIVE clip headroom — no NM body can GROW there without dropping a matched tail fn (2026-07-10)** — _The unit's NM object `.text` already sits ~0x328 OVER its `NON_MATCHING_TEXT_CLIP_KEEP_ALIGN := 0x2b6d0` (salvaged by KEEP_ALIGN). Growing any middle function (e.g. reconstructing a stub like gl_func_00036224/0003A58C) shifts the tail so the fixed clip boundary bisects a later function → asm-processor emits an out-of-bounds symbol and `objdiff-cli report generate` FAILS entirely, OR zeroes a matched tail fn (the clipped tail includes game_libs_func_00062F08 @100%, gl_func_00062E10 @83%, gl_func_00062E80 @81%). So a stub-reconstruction there trades one low-fuzzy rise for dropping a 100% match + two 80%+ = net-negative. To decode a post0b stub you must FIRST do clip-infra: raise 0x2b6d0 to the next CLEAN function boundary past the grown content (and bump it by the growth delta), then re-verify the exact-fn SET is unchanged. Not a single-tick task. Determinate post0b stubs are otherwise decode-able — it's purely the clip wall. (Non-post0b under-reconstructed fns don't hit this.)_
- **NM CLIP value must cover the LAST symbol's full extent — growing NM wrap bodies shift boundary fns into the clip (62F08, 2026-07-10)** — _When an NM body grows (wrap-grind rise), every later symbol in the unit shifts; a NON_MATCHING_TEXT_CLIP_KEEP_ALIGN unit-level value that was "natural size" now cuts INTO the final matched fn's symbol, silently dropping a landed exact to <100 (62F08 went 100→65). Post-cherry-pick check: diff the exact-fn SETS (not just totals) between origin/main's report.json and the refreshed one; a lost name near a clip boundary means re-measure the clip (last-symbol start + its expected size, rounded up to align)._
- **Every new real-path TRUNCATE_TEXT carve needs an NM CLIP twin check post-land (102A4, 2026-07-10)** — _-O0 units emit dead trailing jr-pairs past the function's real end; TRUNCATE_TEXT clips them in the real .o but the non_matching .o keeps them, so objdiff scores a landed byte-exact fn at <100 (102A4 read 88% on identical words). Fix: `build/non_matching/.../<unit>.c.o: NON_MATCHING_TEXT_CLIP_KEEP_ALIGN := <unit-len> <fn>=<real-size>`, force-delete the NM .o, rebuild, confirm 100.0 in report.json._
- **NEW SPLIT-OFF UNITS are silently UNTRACKED until added to objdiff.json — exact matches vanish from report.json (bootup o0 splits, 2026-07-10)** — _Carving a new per-file unit (e.g. `bootup_uso_o0_10B6C.c`) and wiring Makefile+tenshoe.ld is not enough: report generation only walks objdiff.json units, so the new unit's functions disappear from BOTH numerator and denominator (total fn count dropped 3474→3470 and three landed exacts read as absent, not <100). The land script doesn't catch it (it verifies expected-vs-build disasm, not tracking). Fix: clone an existing sibling unit entry (name/target_path/base_path/metadata.source_path + correct c_flags), insert, rebuild NM objects, refresh-report. CHECK: after landing any function in a NEW unit, grep objdiff.json for the unit stem and confirm the fn appears in report.json at 100.0._
- **Object-span .s regeneration REINTRODUCES the SUFFIX-emitted pad word → suffix injector refuses in baseline (W50 quartet, 2026-07-10)** — _Regenerating a merged .s from the built object's symbol span is the authoritative source for the BODY, but if the fn has a `SUFFIX_BYTES_FORCE` entry, the object symbol INCLUDES the pad word the injector appends — dumping the full span and writing it back makes the baseline INCLUDE_ASM emit a tail of (addiu-in-delay, 0x00000000), and inject-suffix-bytes.py refuses ("doesn't end with jr ra+nop"; the check is only insn[-2]==0x03E00008), killing `make objects EXPECTED_BASELINE=1`. Fix: trim the trailing zero word from the regenerated .s so the tail is jr + delay-slot insn (the check passes on any delay content when insn[-2] is jr); baseline then emits body + injected suffix = same layout as the real path. Diagnose with the visible-stderr swap-replication snippet (exec refresh-expected-baseline.py's helpers, run the baseline make with captured stderr, restore in finally)._
...
- [Probe loops MUST gate on cc exit status: a silent NM-build error freezes the .o and every subsequent probe measures STALE bytes (false-inert verdicts); also scope Python replace() edits to the target fn body — replace-all hits sibling fns (redecl break) (675A4, 2026-07-15)](#probe-loop-stale-o-false-inert)
...
- [Reloc PER-SYMBOL COUNT diagnoses CSE-collapse; call-seq-zip-by-POSITION maps placeholders](#reloc-per-symbol-count-diagnoses-cse-collapse-vs-distinct-symbol-call-seq-zip-by-position-maps-placeholders-2026-06-21) — _Compare per-symbol reloc COUNT (not just set) vs expected/.o: target referencing a base symbol N× while your build has 2 (1 HI16+1 LO16) = IDO CSE-collapsed `&SYM+off` into one reused reg vs target's fresh lui/addiu per site (C48C target = D_807FF568 x16 + D_807FF5B0 x16, NOT 35 distinct externs — a prior agent had it backwards). Histogram disambiguates "one base re-materialized" from "many distinct symbols". To map a `_f`/placeholder call, zip built-vs-expected call relocs BY ORDINAL POSITION; first `<<<` divergence pins the symbol AND surfaces missing/mis-ordered calls. scripts/verify-fn.py masks HI16/LO16/CALL26 immediate bits for a clean non-reloc-word-diff + reloc-seq verdict._
...
- [A NONZERO baked `jal 0x0C00NNNN` is NOT a structural cap — map it to `gl_ref_<target>` (call-side), and never call the REAL fn at that addr](#feedback-nonzero-baked-jal-gl-ref-callside-2026-06-22) — _Raw-.word USO `.s` files ship `jal` immediates UNRELOCATED (bake-data-relocs.py: R_MIPS_26 left raw, fixed at load time). A nonzero `jal 0x0C00E89A` decodes via nearest-symbol to e.g. `game_libs_func_X+0xNNNN` and lands MID-function (on an FPU insn, not an entry) — looks like a structural cap, ISN'T. Reproduce it exactly the same way data refs use `gl_ref_X`: add `gl_ref_0003A268 = 0x0003A268;` to undefined_syms and call `gl_ref_0003A268(...)` — the C call emits `jal 0`+R_MIPS_26, ld bakes the absolute → `jal 0x0C00E89A`. PITFALL: do NOT call the real function whose name the disassembler guessed (e.g. gl_func_0001CA10@0x1CA10) for a `jal 0` site — that bakes `jal 0x0C007284`, the WRONG immediate; `jal 0` (the placeholder) must map to `gl_func_00000000` (=0). Same gl_ref mechanism as the matched gl_func_0000408C. Took gl_func_00025C54 46.25→76.7% (residual = s8-vs-spill coloring). 2026-06-22 agent-d._
...
- [Dead-`#else`/`#ifdef NON_MATCHING` bodies are a HARVEST VEIN — the trapped inner body is often the MATCHING one; gate on `make verify` ROM-cmp, not the tautological `expected/`](#dead-else-ifdef-harvest-vein-2026-06-21) — _The nested-dead-code bug leaves a later structural body preprocessor-dead; it often compiles to FEWER diffs (sometimes 0) than the active body. Flip its guard to `#ifndef`, re-test, and gate on `make verify` (`cmp ROM baserom.z64`) — `expected/` is just `cp build/*.o` so word-compare vs it is tautological for a freshly-promoted body. Landed func_80001CF4 (kernel) 2026-06-21. Still-capped: game_libs constructor-family (return-reg-as-base-ptr / unused-arg-home frame-grow, IDO won't emit from C)._
...
- [Symbolizing a CSE-collapsed big USO record-builder constructor REGRESSES report fuzzy — keep the placeholder body (monotonic), validate symbols via ROM reloc table not fuzzy (timproc_uso_b5_func_00004118 72.19→66.40, 2026-06-22)](#objdiff-cli-report-generate-is-reloc-name-blind-only-objdiff-cli-diff-is-name-aware--so-the-reportland-gate-cannot-validate-a-uso-call-target-even-with-symbolized-expected) — _Report gate is reloc-name-blind (resolved value 0==0), so replacing `&D_00000000+off` (one CSE'd lui) with ~40 distinct real `D_807FExxx` name-pool symbols earns ZERO fuzzy credit while forcing a fresh lui/addiu per site — on an already-OVERSIZED body that ADDS ~85 words and desyncs every position-compared word (72.19%/-1 → 66.40%/+85). The real blocker is the FRAME (build 0x720 vs target 0x3A8): the m2c body declares ~42 `Q` 0x20 scratch structs the target overlays/reuses (~2x frame) and never promotes the reused 1.0f to `$f20`. Frame/slot-coloring is the multi-pass grind; until it lands the symbol-correct body scores WORSE. Same class: func_00001F14 (65%, 3 callees + 7 f64/f32 consts; has literal-addr `*(f64*)0xD0` decode bugs)._
...
- [Before grinding a near-miss, raw-word diff it against already-EXACT siblings in the region — a 1-word delta means mirror the proven body verbatim (652D8 66.85→100 in one edit, 2026-07-17); ±1-word extension via expected/.o bytes crossed 4 more (C5B0/551E0/1AFC/453A8, 2026-07-18) + NM-clip stale-size and jr-delay-nop resize traps](#sibling-raw-word-diff-before-grinding) — _If word-identical to a cracked sibling modulo an immediate/base constant, copy the sibling's full lever stack (decl order, volatile pads, BB-split ifs, line grouping) and change only the constant. Extract targets from expected/.o (exacts = absent OR explicit 100.0 fuzzy field); ±1-word twins differ by the delay-nop attribution; after harvest, update NON_MATCHING_TEXT_CLIP_KEEP_ALIGN size and add fn=0xNN symbol resizes; scheduler-swap deltas (lui-first trio) are NOT harvestable._
...
- [Corollary: 3 more field-type-from-mnemonics fixes (s16 READ via lh; f32 global via lwc1 inline-not-CSE'd; FP compare/divide not int)](#m2c-overwiden-struct-store-unaligned-2026-06-22) — _gl_func_00002B94 23.9→51.2% normalized: m2c misdecodes halfword reads as lwl/lwr (narrow to `*(s16*)`), reads an f32 const-data global as `*(s32*)`+cvt (use `*(f32*)&D` lwc1; inline per call site, NO CSE local — target re-emits a load each time), and renders FP field compares/divides (c.lt.s/c.le.s/div.s) as int slt. Correctness wins; residual = regalloc+scheduling cap._
- [DECODE-ONLY reconstruction triage: drive WORD-COUNT to size-EXACT before conceding "coloring cap"](#decode-only-reconstruction-triage-drive-the-word-count-to-size-exact-before-conceding-coloring-cap--a-build-longer-body-usually-still-hides-size-correcting-bugs) — _A build-longer reconstruction hides size-correcting BUGS, not just scheduling. timproc_uso_b5_func_00006394 311→306w (EXACT), 69.1→73.2%, via 4 levers: (1) f32 field vs float literal read as int → kills a cvt (use `*(float*)`); (2) `&D_00000000+off` placeholder really a reloc'd nearby global (import_800200CC+0x34, &D_807FF5E0+0x160) — grep target reloc names; (3) drop redundant explicit `spNN` local (double-store); (4) reorder a call-independent field-read to AFTER the call to kill a spill/reload pair. Residual then = true unpaired-HI16 cap + coloring._
- [Alloc-cascade constructor cap class — m2c renders goto-on-failure as fused `&&`/`||`, regalloc diverges body-wide](#alloc-cascade-constructor-cap-class--m2c-renders-goto-on-failure-as-fused--regalloc-diverges-body-wide) — _Long alloc-or-use chains (func_000090CC: 58 cascades + unrolled stride-24 inits) need the original goto structure rewritten by hand; m2c's fused-boolean form desyncs regalloc body-wide. Multi-session, not a single-tick land. **Corollary:** the body-wide desync MASKS real logic bugs — classify every diff word {renumber/frame/branch=cap} vs {wrong opcode/base/imm/jal-arg=BUG} before conceding; permuter confirms the cap half but never the bug half. h2hproc 620: 3 hidden bugs (fail-arm fall-through not goto-end, a2/a3 arg swap, vtable arg += retval) fixed. Drop shared `&D` local (forces long-lived spill); re-materialize per ref._
...
- [TRUNCATE_TEXT blocks C conversion of asm-padded functions in bootup_uso](#feedback-truncate-text-blocks-c-conversion) — _In 1080's bootup_uso.c (and its tail[1-4].c splits), converting an `INCLUDE_ASM` to C can fail with "`.text is already smaller (0xNNNN < 0xMMMM)`" when the original asm has trailing alignment nops that IDO doesn't… **RESOLVED for the single-trailing-nop sub-case (2026-06-21, func_0000EE8C landed): append the one nop word via `SUFFIX_BYTES_FORCE := <fn>=0x00000000`; works mid-segment with a following symbol, no TRUNCATE bump needed. objdiff still reads ~97% (SUFFIX not on the non_matching path) — gate on ROM cmp, not fuzzy.**_
...
- [A forward-branched return-0 g3-carve leaf is the PARENT's shared epilogue — absorb it, drop the carve](#feedback-absorb-forward-branched-g3-return0-tail) — _When a tiny `-O2 -g3` carve leaf (`move v0,zero; jr ra; nop`) sits immediately after a switch/flag function whose default + `(&X)==0` cases `beq`/`beql` FORWARD into it (targets = the leaf's address / +4), the leaf is not standalone — it's the parent's shared `return 0` epilogue that splat over-split. Writing the parent with an internal `z: return 0;` makes IDO -O2 emit the parent's N words PLUS the 3-word tail byte-identically, with the `jr ra` delay naturally unfilled (the `move v0,zero` is the merge-point value reused by the `beql` delay slots — so no g3 carve is needed for the unfilled form). Land it by absorbing: make the parent the merged (N+3)-word C fn, delete the leaf's `_g3_<off>.c` donor, and drop its entry from the block's g3-splice (deps, OBJCOPY, the `gN`/`p4` term, and arg indices). Gate: block decompress vs ROM asset = exact length + 0 non-reloc diffs. Verified 2026-06-20 timproc_uso_b5_func_000088A0 (absorbed func_00008940). This is the honest inverse of the g3-carve: not every tiny return-0 leaf needs carving — if a sibling branches INTO it, it's already that sibling's tail._
...
- [FALSE-MATCH TRAP: `verify-blocks other=0` + ROM-OK do NOT prove a Yay0-block fn byte-exact — spill/frame offsets hide in `lo16`, ROM links the STORED asset](#false-match-trap-verify-blocks-other0--make-verify-rom-ok-do-not-prove-a-yay0-block-function-is-byte-exact--spillframe-offset-diffs-hide-in-lo16-and-the-rom-links-the-stored-asset-not-the-rebuild) — _Burned + retracted 2026-06-22 (timproc_uso_b1_func_00001A64, "99.96 reloc-presence false-negative" → unwrapped + episode, was actually 4 swapped spill slots). `classify()` buckets any diff with matching high-16 as benign `lo16`, so sp-relative `sw`/`lw` offset diffs pass as `other=0`. And the ROM links `build/assets/%.bin.o: assets/%.bin` (stored ground truth), NOT the rebuilt block, so ROM-identity says nothing. ONLY real gate = decompress the BUILT block + word-diff the fn range vs `assets/<blk>.bin`. The placeholder-form recovery vein is EMPTY: `report generate` already credits placeholder relocs at 100; 0/180 sub-100 USO fns across all 5 code units are pure reloc-presence false-negatives — every one has a real regalloc/spill diff. Don't re-run the scan._
...
<a id="nm-clip-budget-symbol-oob"></a>
## NM clip-budget: growing an NM body in a clipped unit breaks the whole report pipeline (2026-07-18)

`build/non_matching/src/game_libs/game_libs_tail.c.o` has `NON_MATCHING_TEXT_CLIP_KEEP_ALIGN := 0x5530 ...` (Makefile). The unit's NM .text is zero-sum: every undersized NM body leaves headroom, every oversized one spends it. When a redecode grows a body (D9E4 0x144->0x150 target-exact), all later symbols shift and the resized sentinel (gl_func_0000EBC8=0x2c) straddled the 0x5530 cut:

- `objdiff-cli diff` hard-fails ("Symbol data out of bounds: 0x5510..0x553c") — and silently leaves your previous `-o` JSON on disk, so a re-read shows STALE results (looks like your edit did nothing).
- `objdiff-cli report generate` fails the same way, and `scripts/refresh-report.sh` redirects its empty stdout into `report.json`, TRUNCATING it. Restore with `git checkout HEAD -- report.json`.

Fix by rebalancing inside the unit: audit per-symbol `.text` sizes (objdump -t expected vs build/non_matching), find the oversized sibling NM bodies, and make one exact-size (CB9C +0xC absorbed the D9E4 growth — and went word-exact in the process). Iterate with a direct objdump-based word diff while the unit is oversize, since objdiff is unusable in that state.
...
## A NONZERO baked `jal 0x0C00NNNN` is NOT a structural cap — map it to `gl_ref_<target>` (call-side)
<a id="feedback-nonzero-baked-jal-gl-ref-callside-2026-06-22"></a>

The data-ref `gl_ref_XXXX` mechanism (above) applies to CALL targets too — the R_MIPS_26 / `jal` side, not just HI16/LO16 data. Raw-.word USO `.s` files ship `jal` immediates UNRELOCATED (`bake-data-relocs.py`: "R_MIPS_26 and everything else: left RAW — the USO ships jals unrelocated and fixes them via its load-time reloc table").

**Why it looks like a cap (and isn't):** a nonzero baked `jal 0x0C00E89A` disassembles via nearest-symbol to e.g. `game_libs_func_0003443C+0x2283c` and, when you resolve `(0x00E89A << 2) = 0x3A268`, lands MID-function (on an FPU `bc1f`/`c.le.s`, not an entry point). That mid-function landing screams "external/USO-relative jumptable = structural cap, report don't grind." It is a red herring — the immediate is a pre-relocation literal, and you reproduce it exactly like a data ref:

```c
/* undefined_syms_auto.txt */
gl_ref_0003A268 = 0x0003A268;   /* nonzero baked jal 0x0C00E89A */
gl_ref_0003959C = 0x0003959C;   /* nonzero baked jal 0x0C00E567 */
/* C */
extern int gl_ref_0003A268();
gl_ref_0003A268(rec + 8);        /* emits jal 0 + R_MIPS_26; ld bakes -> jal 0x0C00E89A */
```

**PITFALL (the trap that motivated this entry):** for a `jal 0` (zero-immediate placeholder) site, do NOT call the real function whose name the disassembler guessed. e.g. `jal 0` near a `<gl_func_0001CA10>` label is the USO PLACEHOLDER (= `gl_func_00000000`, value 0). Calling the *real* `gl_func_0001CA10` (which lives at 0x1CA10) bakes `jal 0x0C007284` — the WRONG immediate. Rule: `jal 0` → `gl_func_00000000`; nonzero `jal 0x0C00NNNN` → `gl_ref_<NNNN<<2>>`. Same mechanism that lands gl_func_0000408C (`gl_ref_00013C70`). Took gl_func_00025C54 from 46.25% → 76.7% (the residual is unrelated s8-vs-stack-spill register coloring).
...
## m2c-graft gotcha: float immediate stored into an s32-typed slot silently drops the float store

When a raw m2c graft renders an object-field write as `*(s32 *)((char *)p + 0x10) = 1.0f;` (m2c defaults the slot type to s32 but keeps the float RHS), C **narrows the float to an int at compile time** — so IDO emits an integer `li/sw`, NOT the target's `mtc1`/`lui;mtc1` + `swc1` float store. The build comes out SHORTER and the codegen diverges. Fix: retype the lvalue cast to `*(f32 *)`. On `func_000090CC` (bootup_uso, the project's single largest unmatched fn, 2106-insn alloc-cascade constructor) this affected 20 `unk10 = <float>` sub-init slots; fixing them added +17 build words and raised the reloc-filtered align-ratio 0.1553→0.1626. Cheap, safe, mechanical — grep any large m2c graft for `\*\(s32 \*\).*= [0-9]+\.[0-9]+f;` before deeper work. (Note this only matters for IMMEDIATE float RHS; `(f32) *(f32 *)0xNNN` pool reads already emit `lwc1`.)

## DECODE-ONLY reconstruction triage: drive the WORD-COUNT to size-EXACT before conceding "coloring cap" — a build-longer body usually still hides size-correcting BUGS

For a DECODE-ONLY function (raw-word USO with an unpaired-HI16 reloc cap, byte-match impossible) the goal is max fuzzy/LCS, but the right method is the same as for a real match attempt: **first eliminate every size-changing bug; the residual is only then classifiable as the coloring cap.** A reconstruction that emits MORE words than target almost always has fixable bugs, not just "scheduling overshoot" (refines the build-longer entry above for the reconstruction-from-scratch case). On `timproc_uso_b5_func_00006394` (1224B FP-heavy timing-draw helper) this took 311→306 words = EXACT, 69.1%→73.2% LCS, via four bugs each worth 0–2 phantom words:

- **f32 field compared to a float literal read as int.** `FW(p,0x480) == 1.0f` where `FW` is `*(int*)` → IDO loads the slot as int and **converts to float** (`lw; mtc1; cvt.s.w; c.eq.s`, 4 words) instead of the target's direct `lwc1 $f4,N(p); c.eq.s` (2 words). Fix: `*(float *)((char*)p + 0x480) == 1.0f`. (Mirror of the float-IMMEDIATE-store gotcha above, on the LOAD/compare side: a float field touched through an int lvalue costs a spurious cvt.)
- **`&D_00000000 + 0xNN` placeholder where the target relocates a real nearby global.** The `if (*(s32*)(&D_00000000+0x34)==2)` gate actually reads `import_800200CC+0x34` (a HI16-reloc'd global), and a table base written `&D_00000000` was really `&D_807FF5E0+0x160`. Bare `&D_00000000+off` builds (LO16=off) but diverges from the global's reloc; use the real reloc-table symbol. Grep the target's `objdump -dr` reloc names for the slot before trusting a `D_00000000` placeholder.
- **Redundant explicit stack local forcing an extra slot.** A `sp24 = var_a2; … f(…, var_a2)` writes var_a2 to its OWN slot AND lets the compiler spill it for the call = two stores. Drop the explicit `spNN` local; let the arg-spill alone home it.
- **Spill from reading a field BEFORE the call that doesn't need it.** `t = FW(arg0,0x4A4); v = FW(alias(arg0),0x2B0);` makes `t` cross the `alias()` call → `sw/lw` spill pair. The target reads `0x4A4` AFTER the call returns (it only needs `arg0`, still live). **Reorder so the call comes first**, then read the call-independent field — kills the spill/reload pair (2 words). General lever: a local that is call-independent but textually precedes a call gets spilled; move its read past the call.

After these, the remaining ~82 diffs were all SAME-SIZE: the unpaired-HI16 base/array load (target folds `+0x160` into the base `addiu` via reloc with literal-0 `lw` offsets; IDO-from-C emits paired HI16/LO16 and folds `+0x160` into the index `*0x18` multiply instead — genuinely unreproducible from C) plus pure register-renumber coloring + stack-slot cascade + one scheduler load-order swap. THAT is the real cap; the +5 overshoot before was bugs masquerading as it. Always size-match first.

## Alloc-cascade constructor cap class — m2c renders goto-on-failure as fused `&&`/`||`, regalloc diverges body-wide

1080's big USO constructors (e.g. `func_000090CC`) are long chains of "alloc-or-use" cascades: `sub = base+off; if (sub == SENTINEL) { sub = alloc(N); if (!sub) goto <shared join>; } init(sub,…);`. Failed allocs branch FORWARD to shared join labels (a mid-function continue point and the epilogue), not a single early-out. m2c reconstructs every cascade as a fused boolean `if ((sub != SENTINEL) || (t = alloc(N), sub = t, t != 0)) {…}`, which is logically close but changes liveness/spill scheduling enough that IDO's register allocation diverges across the WHOLE body — the build stays structurally desynced (ratio ~0.16) and tens-to-hundreds of words short regardless of per-block fixes. A faithful match needs the original goto structure rewritten by hand for all N cascades (58 + a 12-entry unrolled stride-24 init block on 090CC) — genuinely multi-session, not a single-tick land. Best single-pass outcome is the float-slot fix above + honest NM. Don't expect per-region tweaks to close it.

**Corollary — the body-wide regalloc desync MASKS real logic bugs; classify every diff word before conceding "coloring cap" (h2hproc_uso_func_00000620, 2026-06-22).** A cascade constructor parked at ~77% fuzzy and labelled a `q→$a2`-vs-target-`$a3` renumber cap turned out to have THREE genuine logic errors hiding in the misaligned region: (1) alloc-fail arms wrote `goto end` instead of falling through to the matching `*(x+0x28)=&D` store (the target's fail branches land on the `6f8/6e4/6d4` store-section labels — alloc-OR-passthrough cascade with dead arms, NOT alloc-or-bail); (2) an arg swap (`gl_func(p,&D,a1,a2)` + `*(p+off)=a3`, not a2/a3 transposed — read the arg-spill slots: a1@sp+0x3c, a2@+0x40, a3@+0x44, then see which slot each later `lw` pulls); (3) a virtual-call arg was `(short)vt[0x58]` when the jalr-delay `addu a0,t5,v0` proves it's `(short)vt[0x58] + retval_ptr`. None changed fuzzy (all sat inside the already-desynced span) but all are real correctness fixes worth landing in the NM body + migration value. **Workflow: when a fn is dismissed as a coloring cap, still walk the diff and split each differing word into {register-renumber, frame-slot, branch-offset} (cap) vs {wrong opcode, wrong base reg, wrong immediate, wrong jal arg-setup} (BUG).** The permuter confirms the cap half (h2h 620: base 159650, floor 158080 over ~660 iters with tuned reorder/temp/split weights — local minimum, no crack) but it can NEVER fix the bug half; only manual diff-word classification does. Also: drop any shared `char *base = &D` local — it forces one long-lived spill across the whole body; let each `&D` ref re-materialize (use distinct extern symbols for distinct `*(x+0x28)=&D` stores so IDO emits separate `lui/addiu` like the target instead of CSE-ing to one register).
...
**NEGATIVE RESULT — the game_libs "RELOC-MISMATCH" / "symbolization-blocked" near-miss band is NOT C-fixable; it is regalloc/schedule residual (2026-06-21, agent-e, byte-verified).** A prior pass flagged ~86 game_libs near-misses (95-99.9%) as "RELOC-MISMATCH": hypothesis was that `expected/.o` BAKES a real non-zero address with NO reloc while the C-emit emits a placeholder `&D_00000000`/`gl_func_00000000` reloc, so giving the C the real address (`*(T*)0xADDR` literal deref, or a named sym) would land them. **This is false.** Evidence: (1) `expected/src/game_libs/*.c.o` is just a snapshot COPY of the INCLUDE_ASM build of the raw-`.word` `.s` files (Makefile `expected:` target = `cp build/src/.../*.o expected/`). game_libs is a RELOCATABLE module: the **baserom itself** ships zero immediates in every lui/addiu/jal (`3C020000`, `24420000`, `0C000000`) plus a SEPARATE runtime reloc table — verified by dumping `baserom.z64` at the function's ROM offset. So expected has **0 relocs and 0 immediates**; there is no baked address to recover. (2) The C-emit produces the SAME zero bytes + R_MIPS_HI16/LO16/26 placeholder relocs; objdiff is reloc-aware and scores the placeholder-vs-zero as MATCHED — which is precisely why these sit at 95-99.9%, not far lower. (3) Word-comparing `build/non_matching/.o` vs `expected/.o`: `game_libs_func_00026B40` = 2 diffs, both a register-materialization choice (`lui a0;addiu a0,a0` mine vs `lui t6;addiu a0,t6` target); `gl_func_00033B6C` = 4 diffs, the four `addiu sN,sN,imm` loop-pointer inits emitted in REVERSED order (pure IDO schedule/decl-order). Neither diff is in a reloc. (4) Band-wide scan of all **83** game_libs near-misses (95-99.99%, size≤400): **ZERO** have a baked-nonzero-immediate-vs-placeholder mismatch (9 are size-mismatch/EXTRA_INSN, 74 are same-size regalloc/schedule). (5) The proposed `*(T*)0xADDR` "fix" would emit `lui reg,0x800B`-style NONZERO immediates with no reloc, which mismatch the expected `lui reg,0x0` at every touched word — strictly WORSE. **Verdict: the band is the ordinary IDO regalloc/scheduling cap (per-fn structural RE only), not a symbolization problem. "RELOC-MISMATCH" was a misnomer — the relocs already match. Do not re-attempt address-symbolization on game_libs near-misses.**
...
### De-alias into the REAL reloc-table symbols — the big DECODE-% lever for raw-word USO (5.63%→75.30%)

For raw-word USO functions, large NM decodes often paraphrase **every**
call/global through one catch-all extern (`<seg>_alias()`, `gl_func_00000000`).
That collapses many distinct `R_MIPS_26`/`HI16/LO16` targets to one symbol and
scores **terribly** (timproc_uso_b5_func_00005FC0 was 5.63% reloc-masked, 284
insns vs 245 — the all-`alias` form even shifts the whole frame).

**The distinct symbol names ARE recoverable** — they live in the
`expected/src/<seg>/<seg>.c.o` reloc table. Get them with
`mips-linux-gnu-objdump -dr expected/.../<seg>.c.o`: each `jal`/`lui` prints its
`R_MIPS_26 <name>` / `R_MIPS_HI16 <name>` annotation (e.g. `import_80020098`,
`import_000A7ECC`, `timproc_uso_b5_func_00007078`, the per-state jump-table
handlers). **Declare those exact names as externs and call them directly** —
each distinct callee → distinct extern, each global → `extern char SYM;` used as
`&SYM` (or `&SYM + off`). The relocs then resolve to the same symbols the
target uses → reloc-masked match jumps. This is the inverse of the
`&D_00000000`-CSE rule above: when the **real** names are known (USO sidecar /
expected reloc table), prefer them; `&D_00000000` is only the fallback when the
name is genuinely a placeholder.

Gotchas hit: (1) callees defined elsewhere in the same TU already have real
prototypes — forward-declare matching their existing signature (return type!),
don't re-`extern` with a conflicting one (IDO errors `redeclaration` /
`Incompatible function return type`). (2) int-returning callees assigned to
`char *` need an explicit `(char *)` cast. (3) FP field types from mnemonics:
`lh`=`s16` (the 0xB8 vtable base), `lwc1`=`f32`. (4) FP reduction `var*const`:
write **const-first** (`const * var`) to match IDO's load-const-then-var reg
coloring (nudged one more diff off). Residual cap = m2c-vs-IDO jumptable
...
**Corollary (2026-06-22, agent-e): symbolizing a CSE-collapsed big USO record-builder
constructor REGRESSES the report fuzzy — do NOT chase it as a %-mover.** Because the
report gate is name-blind (above), replacing `&D_00000000 + off` (one base, CSE'd to a
single `lui` the whole function reuses) with the ~40 distinct real `D_807FExxx` name-pool
symbols gets ZERO fuzzy credit (resolved value is still 0) while FORCING a fresh
`lui`/`addiu` per site — which the target genuinely has, but on a body that is already
oversized this ADDS instructions and DESYNCS every downstream position-compared word.
Measured on `timproc_uso_b5_func_00004118` (6872 B, 1718-word record-builder; allocs
0x4F0/0x2C/0x38C/0x8 then 0x18×10 + 0x24×13 + 0x18×8, each via `func_055750`/`func_04A188`):
placeholder body = **72.19% / -1-word size gap**; full real-symbol substitution (calls
`func_055750`/`func_04A188`/`func_04C678`/`func_000087A0`/`func_00008D90`/`func_06970C`/`func_04C774`;
the EAD0..EB68 name pool; descriptors `D_807FD9C8+0x158`/`DA10+0x1A0`/`D9F8+0x188`) =
**66.40% / +85-word size gap** (the +85 ≈ the un-CSE'd per-site luis). The real blocker is
the FRAME (build `0x720`/-1824 vs target `0x3A8`/-936): the m2c body declares ~42 `Q`
(0x20) scratch structs the target overlays/reuses, ~doubling the frame, and never promotes
the reused 1.0f to `$f20` (target prologue `swc1 $f20`). That frame/slot-coloring is the
multi-pass grind; until it lands, the symbol-correct body scores WORSE, so the right move
is to keep the placeholder body (monotonic rule) and validate any future real symbols via
the ROM reloc table per the recipe above, not via fuzzy. Same class applies to
`func_00001F14` (65%, 3 distinct callees `func_000087A0`/`func_00001DB0`/`func_04CD94` +
7 f64/f32 consts `D_807FF0F8..F130`; its m2c body also has literal-address `*(f64*)0xD0`
bugs that ARE worth fixing but, being reloc-loads, also earn no report credit).

---

---

<a id="feedback-absorb-forward-branched-g3-return0-tail"></a>
## A forward-branched return-0 g3-carve leaf is the PARENT's shared epilogue — absorb it, drop the carve

_The inverse of the `-O2 -g3` mid-block carve (see feedback-replace-func-body-o0-donor): not every tiny `move v0,zero; jr ra; nop` return-0 leaf needs its own carve. If the function immediately BEFORE it branches FORWARD into it, the leaf is that function's own shared `return 0` epilogue that splat over-split into a separate symbol._

**Recognition.** Disassemble the parent (a switch/flag decoder). Its default/out-of-range `beq at,zero,<addr>` and its `(field & X) == 0` cases `beql tN,zero,<addr+4>` target addresses that land in the NEXT symbol — the 0xC return-0 leaf (`or v0,zero,zero; jr ra; nop`). The leaf's first word is the `move v0,zero` reached by the `beq`; the second word (`jr ra`) is the `beql` target (the `beql` carries `or v0,zero,zero` in its own annulled delay slot, so it jumps straight to `jr ra`).

**Why -O2 reproduces it without a carve.** Write the parent with an internal label epilogue:
```c
int parent(char *a0) {
    switch (*(int *)(a0 + OFF)) {
    case 1: if (...) return 8; goto z;
    ...
    }
z:  return 0;
}
```
IDO -O2 emits the parent's N words PLUS the 3-word `move v0,zero; jr ra; nop` tail at the end, byte-identically — and the `jr ra` delay is naturally **unfilled** even at -O2, because the `move v0,zero` is the merge-point value reused by the `beql` delay slots and can't be sunk into the delay. So the g3 carve (whose only purpose was forcing the unfilled `jr ra`) is unnecessary here.

**Landing (absorb + drop the carve).** (1) Make the parent the merged (N+3)-word C function (unwrap any `#ifdef NON_MATCHING`/`#else INCLUDE_ASM`). (2) `git rm` the leaf's `<seg>_g3_<off>.c` donor. (3) In the Makefile, remove the leaf from: the g3 `OPT_FLAGS` line, its two `TRUNCATE_TEXT` lines, the block's prereq list, the `OBJCOPY ... $(word K,$^)` line, and the splice python (drop the `gN`/`pN` term and renumber `sys.argv` indices + the `n=int(sys.argv[...])` size arg). The splice offsets of EARLIER carves are unchanged. (4) Gate = block decompress vs ROM asset: exact length + 0 non-reloc word diffs (the parent's jumptable `lw $t7,off($at)` LO16 stays a reloc-class diff). The block size (`YAY0_TEXT_SIZE`) is unchanged: the 3 words the parent gained = the 3 words the dropped carve would have inserted.
...
_Refutes the "reconstructed real-symbol USO fn is byte-exact, the <100 fuzzy is just reloc-presence false-negative → unwrap + episode" recovery recipe. The two gates that supposedly prove byte-exactness are BOTH blind to register-allocation diffs in Yay0 code blocks._

**The trap (burned 2026-06-22 on `timproc_uso_b1_func_00001A64`, retracted).** A near-miss USO fn at report-fuzzy 99.96 was unwrapped + given an episode, claimed byte-exact because `make verify-blocks` showed `other=0` and `make verify` showed ROM byte-identical. Both gates LIED. The fn had 4 genuine swapped-spill-slot words (`sw v0,0x20(sp)` vs build `0x30(sp)`; `sw a1,0x34(sp)` vs `0x24(sp)`).

**Why `verify-blocks other=0` is fooled.** `scripts/verify-yay0-blocks.py:classify()` buckets a word diff as `lo16` (treated as benign reloc-class) whenever the high 16 bits match: `if (o>>16)==(b>>16): return "lo16"`. A spill-slot offset lives in the **low 16 bits of `sw`/`lw`** — so `afa20020` vs `afa20030` has identical high half and is misclassified `lo16`, NOT `other`. `other=0` therefore does **not** imply byte-exact; it only means "no diff outside jal-target / lo16-immediate fields." A real frame-offset / register-allocation diff that happens to differ only in a lo16 immediate passes silently.

**Why `make verify` ROM-OK is irrelevant for Yay0 code blocks.** Two pattern rules build `build/assets/%.bin.o`: `: assets/%.bin` (the **stored** ROM-extracted ground-truth asset) and `: build/assets/%.bin` (the **rebuilt** block). The linker script pulls the stored-asset rule, so the ROM contains the original block bytes, NOT your recompiled code. Proof: `mips-linux-gnu-objcopy -O binary --only-section=.data build/assets/<blk>_yay0.bin.o` == `assets/<blk>.bin` (≠ `build/assets/<blk>_yay0.bin`, which can even differ in compressed length). So `cmp tenshoe.z64 baserom.z64` clean says nothing about whether the rebuilt block matches.

**The ONLY real byte gate for a Yay0-block fn:** decompress the *built* block and diff the fn range against the ROM asset:
```python
import crunch64, struct
orig  = open("assets/<blk>.bin","rb").read()
built = crunch64.yay0.decompress(open("build/assets/<blk>_yay0.bin","rb").read())
off, size = 0x<func_block_off>, 0x<func_size>   # from the .s header/2nd column
print([hex(j) for j in range(off,off+size,4) if orig[j:j+4]!=built[j:j+4]])  # must be []
```
Empty list = byte-exact. Any entry (even a "lo16-class" one) on a non-reloc instruction (sp-relative `sw`/`lw`, an `addiu` immediate, etc.) is a REAL diff.

**Corollary — the placeholder-form "recovery vein" is empty.** `report generate` already credits the placeholder reloc form (`gl_func_00000000`, `&D_00000000+off`) at 100 (it's resolved-value-blind to reloc names — see the `report generate` reloc-NAME-blind entry above). A scan of all 5 Yay0-code units (game_uso, timproc b1/b3/b5, mgrproc) for fns that are report-<100 with ZERO non-reloc instruction diffs found **0** across 180 sub-100 fns: every report-<100 USO fn has a REAL instruction-byte diff (spill slot / regalloc / operand order), none are pure reloc-presence false-negatives. No symbol-form change can move a stack offset. Don't re-run this recovery scan.
...
**func_80005520/osCreatePiManager ROOT-CAUSE = -O1 vs -O2 store-base CSE (CONFIRMED 2026-06-23, objdiff 77.43%, NOT landable at -O1).** Modeled the devmgr block as one `PiDevMgr` struct at `D_8000A450` (offsets +0..+0x18: active/cmdQueue/evtQueue/thread/acsQueue/dma/edma) — semantically exact, m2c-confirmed C shape, but objdiff stayed at 77.43% (~67/114 insns diverge). The residual is PURE instruction scheduling + register coloring of the contiguous devmgr-store cluster: the target batches all address-computations then issues the stores sharing ONE `%hi(D_8000A450)` base (the -O2 store-base CSE), whereas kernel_006 is pinned `-O1` (Makefile:297) which reloads `lui at` per store and shuffles the register/order. Generalizable lesson: **a contiguous struct-store block that shares a single `%hi` base across many fields in the target is an -O2 signature; at -O1 IDO recomputes the base per store and you cannot force the batch from straight C** (taking the address into a local `T *p = &g;` made it WORSE — 79 diffs, perturbs the frame). Don't burn ticks restructuring the C; it needs a per-file -O2 split (and even then coloring must line up) or the permuter — but permuter import is blocked here by the NM-wrap + asm-processor two-phase build (import.py "Failed to find GLOBAL_ASM"). Leave INCLUDE_ASM. NOTE: the `.s` target merges osCreatePiManager (0x80005520..0x800056E8 `jr ra`, 114 insns, ROM-verified contiguous) with a trailing `alabel D_800055F0` — `D_800055F0`=0x800055F0 is referenced externally (kernel_030.c, func_80007EEC) as a thread-entry fn-ptr but actually points MID-function (insn `addiu t2,t2,%lo(pimgr_bss_0000)`); it resolves via undefined_syms_auto.txt absolute, so a single contiguous C function is byte-correct.
...
**±1-WORD EXTENSION (2026-07-18, agent-h; 4 crossings in one pass):** widen the pool to exacts one word longer/shorter — the extra word is almost always the trailing jr-delay nop attributed differently (fn-owned vs `_pad.s`/next-fn alignment). Extract every fn's target words from `expected/src/**/*.o` (nm symbol table + objcopy .text) instead of .s files — report.json exacts can carry EITHER no `fuzzy_match_percent` field OR an explicit `100.0`, and landed fns keep .s only sometimes. Hits: gl_func_0000C5B0 86.1→100 (0-delta vs exact 88B4's 35 body words — the old struct-typed wrap was never updated after 88B4's 2026-06-21 flat-int* crack); gl_func_000551E0 65.3→100 (1-word twin of the exact 35A18 dispatcher family, selector li 0x5C vs 0x28); h2hproc 1AFC 92.3→100 (body already exact — the NON_MATCHING SUFFIX mirror line simply omitted its pad word); gl_func_000453A8 22.4→100 (3-word delta vs exact 24330 exposed the real ANSI (int,float,float) signature, floats in GPRs, single-float prototyped callee via a `gl_func_00000000_f` blank-extern alias). Counter-case: a delta that IS a scheduler swap (333F4/3341C/4D05C lui-first trio) is NOT harvestable — see IDO_CODEGEN tiny-wrapper entry.

**Post-harvest bookkeeping traps (same pass):** (1) replacing a long wrong NM body with a shorter exact one SHRINKS the NM .o's .text below a `NON_MATCHING_TEXT_CLIP_KEEP_ALIGN` size → asm-processor hard-errors "smaller than requested" AND sentinel fns (55B10/EBC8 class) flap off 100 until you update the clip constant (post0b 0x2b76c→0x2b750). (2) a word-exact compiled body whose target symbol excludes the jr-delay nop (nop lives in `_pad.s`) scores 30/31-class sub-100 in objdiff until you add `fn=0xNN` to the same clip line (ROM-neutral symbol resize, 55B10 precedent). (3) promoting such a pad.s fn to plain C breaks the linked layout BOTH with the pragma kept (+4) and dropped (−4) — keep the NM wrap even at word-exact (CC14 gotcha, IDO_CODEGEN). **Corrections (2026-07-18, 551E0 land follow-up):** (4) the clip TOTAL is measured on YOUR tree — after landing, other agents' shrinking NM bodies make the pin stale, the clip hard-errors, and (because the .o doesn't depend on the Makefile) a STALE unclipped .o silently survives → the landed fn reads sub-100 on main (551E0 96.77) until you rm the NM .o, re-measure `.text` size, and re-pin. (5) the `fn=0xNN` resize is ONLY for NM asm-pad-split fns whose expected/.o symbol is already the clipped size (551E0/EBC8); for a REAL matched C def (episode landed, e.g. 55B10) BOTH objdiff sides compile from the same C at 0x30 — an NM-side-only resize to 0x2c breaks parity (100→91.67). Classify by episode presence before adding a resize.
...
**HARD LIMIT — a 1-WORD trailing pad followed by another function is NOT reproducible (2026-06-21, bootup_uso func_0000EE8C):** asm-processor reserves each `GLOBAL_ASM` block's space with an empty dummy C function (`void _asmpp_funcN(void){}`), whose -O2 compiled floor is **8 bytes** (`jr ra; nop`). When the asm block is smaller than the dummy, asm-processor back-fills the slot to the dummy size with trailing nops — so a 1-word (4-byte) pad becomes 8 bytes, pushing the NEXT function 4 bytes too late. This is the silent "generates too much" case (asm-processor can't detect it; it's indistinguishable from a static symbol). The 1-word `_pad.s` sidecars elsewhere in the tree only work because they are at **segment end** (no following symbol to be displaced). **Consequence:** a function whose only residual is a single trailing alignment nop (size 0xN0 code + 1 nop = 0xN4, with another function immediately after) is BYTE-EXACT in body but UN-LANDABLE — keep it `#ifdef NON_MATCHING` with a byte-exact body and document. (func_0000EE8C body was made byte-exact via the interleave-decl spill-slot lever — `ret` declared first → highest slot 0x24 — but the lone 0xEF1C nop before func_0000EF20 blocks the land.) A sub-8-byte trailing-pad mechanism (e.g. attaching the nop to the function's own `.size`, or a linker `.size`-patch) would be needed.
...
**RESOLUTION for the SINGLE-trailing-nop sub-case (2026-06-21, bootup_uso/func_0000EE8C LANDED):** when a byte-exact C body is blocked by *exactly one* trailing alignment nop and a *following symbol* (NOT segment-end), append the nop as one all-zero word via `SUFFIX_BYTES_FORCE := <fn>=0x00000000` (matching-path Makefile var, applied in the `build/src/%.c.o` recipe with `--allow-natural-epilogue`). This does NOT need a TRUNCATE bump: the old INCLUDE_ASM already reserved the nop's 4 bytes, so restoring them via SUFFIX leaves every downstream function at its correct offset and the ROM byte-identical. Contradicts the older "1-word pad sidecars only work at segment-end / GLOBAL_ASM dummy overshoots by 4" belief — that was about GLOBAL_ASM `#pragma` dummies (8-byte -O2 floor), NOT SUFFIX_BYTES_FORCE, which appends EXACTLY the words given. FORCE (not plain SUFFIX_BYTES) because the body ends `jr ra; nop`, which trips the plain-SUFFIX natural-epilogue skip path. Same mechanism already in use for gui_func_0000161C and h2hproc fns; func_0000EE8C confirms it works mid-segment. **Gotcha:** objdiff still scores the function ~97% because the SUFFIX is applied only to `build/src` + `expected/`, NOT `build/non_matching` (which objdiff reads as `base_path`) — gate on the ROM `cmp` and a build/src-vs-target byte compare, NOT the report.json fuzzy. Verify with `mips-linux-gnu-objcopy -O binary --only-section=.text build/src/<seg>.c.o /dev/stdout` sliced at the symbol addr+size vs the `.s` `/* … HEX */` words.

**Signal for this sub-case:** asm size = code + exactly 4 bytes (one nop), the function ends `jr ra; nop`, and the next symbol needs 8-byte alignment (fn_end not 8-aligned). Distinguish from the 3-nop F1B4 case (handled differently) by counting trailing `.word 0x00000000` in the `.s`.
...
<a id="dead-else-ifdef-harvest-vein-2026-06-21"></a>
## Dead-`#else`/`#ifdef NON_MATCHING` bodies are a HARVEST VEIN — the trapped inner body is often the MATCHING one; gate on `make verify` ROM-cmp, not the tautological `expected/`

_The nested-dead-code bug above (`#ifdef NM <bodyA> #else #ifdef NM <bodyB> #else INCLUDE_ASM`) leaves `bodyB` preprocessor-dead. But `bodyB` (a later structural reconstruction someone parked in the dead branch) frequently compiles to FEWER diffs than `bodyA` — sometimes to ZERO. Grep these out and re-test each trapped body as the active path. 1080 had ~7 such sites (`grep -n "#else" src/**/*.c` then check the next non-blank line is `#ifdef NON_MATCHING`, not `INCLUDE_ASM`)._

**Recipe to test a trapped body:** flip its guarding `#ifdef NON_MATCHING` → `#ifndef NON_MATCHING` (so it becomes the default-build path) while leaving the outer `#ifdef NM <bodyA>` as-is (`#ifdef`, skipped in the default build). Build `make build/src/<seg>/<unit>.c.o RUN_CC_CHECK=0`, reloc-filtered word-compare vs `expected/.o`. If 0 non-reloc diffs + reloc-set match → unwrap to a single clean body and confirm with the REAL gate.

**THE REAL GATE — not `expected/`:** on 1080 the `expected/` target just `cp build/src/*/*.o` (see the `expected:` Makefile rule), so `expected/.o` == the *last landed* `build/.o`, and a word-compare against it is **tautological/circular** for a freshly-promoted body (the warned-about self-poisoning trap). The authoritative gate is `make verify`, which does `cmp $(ROM) baserom.z64` — the ROM is byte-identical to baserom (rom-exact since 2026-06-10), so a promoted C body that keeps `cmp` clean genuinely matches the original bytes (covers Yay0-compressed USO segments too, since the compressed bytes must match). Always finish with `make verify` (+ `make verify-blocks`) before committing, and add the episode.

**Landed this way (2026-06-21, agent-b):** `func_80001CF4` (kernel, 0xDC/55 insns) — the matching body sat in the dead inner `#else/#ifdef` branch; its 3-slot resource-finalize shape (per non-NULL handle: `func_80001EDC(self,handle,paramA,paramB)` then `func_80000518(handle,paramC)` ALWAYS — commit emitted in the `beql s1,0,next` delay slot, BEFORE the failure test — return `-0xE/-0xF/-0x10`) is what produces the target's branch-likely chain; the active outer body was the wrong (return-before-commit) shape. ROM stayed byte-identical to baserom.

**Caps confirmed still-stuck (don't re-grind):** the game_libs constructor-family dead-`#else` bodies (`gl_func_00038D64`, `gl_func_0003E238`, `gl_func_000688F8`) are NOT crackable by promotion alone — they need a register-web the C can't force: a *return-register reused as a base pointer* for a late `[0x18]` RMW (`addiu v0,a0,0x18` mid-fn + `move v0,a0` return), or a *frame-grow from an unused-arg home spill* (target saves the never-read param a0 to its `0x20` home, frame 0x20 vs built 0x18). IDO folds the named base-pointer back to direct `off(a0)` addressing and won't home an unused arg from straight C — documented frame-layout/scheduling caps, not promotion-fixable.
...
> **CORRECTION 2026-06-20: the original 2026-05-08 titproc claim below was a BANNED-FAKE artifact, not a real sidecar win.** The "6 trailing dead-code words (`jr ra; sw a0,0(sp); nop; nop; .word 0x1; .word 0x7F8; j 0x19420`)" did NOT exist in titproc 2980's real `.s` — they were instruction bytes a reverted land (`4b3b5e3c3`) stuffed into `_pad.s` to FAKE the function tail (the banned post-CC pad-force path, removed 2026-05-23). titproc 2980's true `.s` ends `jr ra; nop` like every sibling, and its real `_pad.s` is ONE all-zero alignment word. titproc was NOT "easier" than mgrproc — they are byte-identical twins. The real lever for BOTH is the sibling-port (see the `feedback-port-matched-sibling-c-...` section below): porting eddproc/mgrproc's two-web body (p1/p2 split, decl order p2,head,p1, arg0[0x40] reload) makes titproc 2980 match from C alone. **Verified 2026-06-20: titproc 2980 landed C-only (build/.o .text == expected/.o .text, 144 bytes, 0 non-reloc diffs), make=ROM OK, verify-blocks all OK.** Do not use this section to justify stuffing instruction bytes into a `_pad.s`. The sidecar may carry all-zero ALIGNMENT data only; arbitrary trailing dead-code that is actually part of the function must be reproduced from C.

**~~Verified 2026-05-08~~ (RETRACTED — see correction above)** — sibling pair:
- `mgrproc_uso_func_00003358` (alloc-and-link): NM 89.22% with documented frame/regalloc cap claim. RESOLVED 2026-06: sibling-port from eddproc, byte-exact.
- `titproc_uso_func_00002980`: prior "matched via `_pad.s` dead-code sidecar" claim was the banned fake (reverted `4b3b5e3c3`). RESOLVED 2026-06-20: sibling-port, C-only byte-exact.
...
## Reloc PER-SYMBOL COUNT diagnoses CSE-collapse vs distinct-symbol; call-seq-zip-by-POSITION maps placeholders (2026-06-21)

When auditing a large NM-wrap against `expected/.o`, compare the **per-symbol reloc COUNT** (HI16/LO16 and CALL26) between built and expected, not just the symbol SET. A symbol that target references N times but your build references twice (1 HI16+1 LO16) means **IDO CSE-collapsed `&SYM + off` into one register** that it reuses across all N sites, while the target re-materialized a FRESH `lui/addiu` per site. Worked example (game_uso_func_0000C48C, a 32-stage unrolled constructor): target HI16 histogram is `D_807FF568 x16` + `D_807FF5B0 x16` (one base symbol, fresh lui per stage), but the NM body emitted only 2 each — and a *prior* agent had mis-diagnosed this as needing 35 DISTINCT per-stage externs (`D_807FF81C..898`), the opposite of the truth. The histogram (count, not set) is what disambiguates "one base re-materialized" from "many distinct symbols". The +Ninsn gap to close it is the CSE-bust, but per the register-pressure caveat (this doc's `feedback-permuter` / IDO_CODEGEN 3281/3188) distinct-extern bust does NOT hold under high pressure (800+ insn bodies) — record the target histogram and defer to a regalloc-dump pass rather than cluttering the body.

To map a synthesized `gl_func_00000000`/`gl_func_00000000_f` placeholder to its real callee, **zip the built call-reloc sequence against expected BY ORDINAL POSITION** (both sorted by offset), printed side-by-side. The first `<<<` divergence pins it: e.g. built idx7=`00003ED4` vs target idx7=`055750`,idx8=`00003ED4` revealed (a) the `_f` placeholder = `game_uso_func_00003ED4` (confirm by arg shape: 3 vec ptrs, returns float) AND (b) a MISSING alloc(055750) before it (a dead-sentinel copy-or-alloc-into-scratch idiom). Positional zip beats name-set diff because it surfaces ordering/missing-call structure, not just which names are absent. Tooling: a small reloc-filtered word-compare + reloc-seq checker (`scripts/verify-fn.py` in the 1080 worktree) masks the immediate bits of HI16/LO16/CALL26 words so placeholder addends don't count as diffs, giving a clean "non-reloc word diffs + reloc-seq MATCH/MISMATCH" verdict per function.
...
## game_libs-internal masked-twin vein is EXHAUSTED — no unmatched fn shares even an opcode-shape with a matched donor (2026-06-21)

Ran a full masked-shape sweep over all ~2124 game_libs `.s` functions
(`scripts/find-masked-twins.py` and `scripts/find-fuzzy-twins.py` — both
added this run). Method: per-function masked signature keeping opcode +
register STRUCTURE, masking immediate/offset/branch-target/jal-target bits;
group; cross-reference match-state from `report.json` (ground-truth fuzzy)
AND from source (clean C body vs active INCLUDE_ASM).

Results (game_libs, 966 matched / ~1066 unmatched real fns):
- **0 donor+unmatched clusters** at exact masked-shape (register-exact).
- **0 near-twins** down to 88% masked-word agreement.
- **0 register-renamed twins** even with the LOOSEST signature
  (opcode-position only, no register numbers) cross-referenced against the
  80-99.99% near-miss band.
- 92 clusters are donor-only (all members already matched — the easy
  accessor/setter/wrapper twin families were fully harvested by prior
  sessions). 26 clusters are plain-only (unmatched twins of EACH OTHER with
  NO matched donor).

The plain-only clusters CANNOT be twin-ported (no donor body to copy); they
require reconstructing one member from scratch, and the ones checked are
documented caps:
- 4-member 32w cluster (gl_func_00035A18 / 0003A044 / 0003EBDC / 0003EC5C):
  byte-identical vtable-dispatchers, decoded to 69.5%, stuck on a
  register-allocation cascade ($t7 vs $a3, base-ptr v1=a0+8). Regalloc cap.
- 7w pair 00009A50 / 00009A80: `bnel` branches PAST function end into a
  shared cross-fn epilogue (feedback_leaf_branch_past_end_is_cross_fn_epilogue).
- 9w pair 0006C454 / 0006F35C: osWritebackDCacheAll — `cache` instruction,
  libgcc-class, not emittable from C.
- 10w pair gl_func_000333F4 / 0003341C: `sw ra; lui a0` scheduler-order swap
  (feedback-ido-o2-tiny-wrapper-unflippable), 13+ variants failed on sibling.

The high-% standalone near-misses (000276B8 @ 95.83%, 00029C80, 0001FE88) are
cross-function-register-continuation caps: callers `jal` into the parent
block's tail so the standalone fn's temps continue the parent's register
counter — the standalone-C ceiling is below 100% by construction.

**Takeaway:** do NOT re-run a game_libs masked-twin / byte-twin port sweep.
The vein is dry; remaining unmatched game_libs fns are genuine regalloc /
...
**Corollary — three more field-type-from-mnemonics fixes for m2c-graft tightens (gl_func_00002B94, 2026-06-22 agent-e, normalized opcode-stream match 23.9%→51.2%):**
...
## Probe loops must gate on compile status; replace-all contaminates sibling fns (675A4, 2026-07-15) <a name="probe-loop-stale-o-false-inert"></a>
Two compounding traps in a C-spelling probe loop (edit -> make <nm .o> -> word-diff):
1. **Silent build break = stale .o = false-inert.** `make ... >/dev/null 2>&1` swallows cfe errors; the old .o stays on disk and the diff harness happily measures it. On 675A4 one broken probe (duplicate decl injected into a SIBLING function) froze the .o and five subsequent levers were recorded "inert" that were never compiled. Verdicts had to be re-derived from a clean re-apply. Gate every probe on `make ... 2>&1 | grep -ci error` (or exit status) and print it next to the diff count.
2. **Scope source edits to the function body.** Python `str.replace()` is replace-ALL; short decl lines (`    f32 var_f0;`) and common statements exist in sibling m2c-style fns in the same TU. Slice `s[i:j]` between the fn's `void fn(...)` and its `INCLUDE_ASM` line, edit the slice, reassemble. A `re.sub(count=1)` "first occurrence" is NOT scoping — the first occurrence may be in another function entirely.

## replace_all bleed into exact-matched siblings; exact-set diff is the only reliable catch (F948, 2026-07-15) <a name="replace-all-bleed-exact-sibling-f948"></a>

Two agents in one wave (2026-07-15) hit the same failure: a `replace_all`-style edit
(`float scale;` → `register float scale; + register float rx, ry, rz;`) intended for one
function silently edited every textual twin in the TU — including the **exact-matched,
default-build-compiled** `game_uso_func_0000F948`, regressing it 100 → 99.79 on main-bound
commits. NM-wrap collaterals (24BC/591C) are harmless (default build is INCLUDE_ASM), but a
compiled-C exact regresses the ROM-adjacent report.

Compounding gotcha: the authoring agent "verified" the regression pre-existed on origin/main
via a stash test and reported it as a **stale-main-report anomaly**. The stash test was wrong.
The orchestrator's bisect that settled it: `git checkout origin/main -- <file>` + rebuild NM +
re-measure → 100.0, proving the commit caused it, then revert just the collateral hunk.

Rules:
- Never `replace_all` on a declaration/statement string that can exist in multiple functions;
  edit with unique surrounding context per site.
- The **exact-set diff vs `origin/main:report.json` gates every push** — it is what caught this.
- Treat agent integrity claims about main ("report lists X exact but it isn't") as unverified
  until reproduced by the file-revert bisect above; prior false alarm class: stale mid-run fetch
  (see project_1080_agentf_report_source_mismatch memo).

## Clip re-measure: `objdump -h` on a clip-built .o is TAUTOLOGICAL — use the shrink hard-error or symbol extents (44EDC land, 2026-07-15) <a name="clip-natural-size-tautology"></a>

When re-measuring a unit's natural NM `.text` after a body-size change, `objdump -h` on the
already-built `.o` just returns the CLIP value you built with (KEEP_ALIGN pads up to it) — it
looks like "natural == clip, nothing to do" while the last function is silently truncated
(62F08 read 65.0 with its symbol cut 0x50→0x34). Two correct measurements:
1. **Probe upward**: set the clip higher than plausible; the build then hard-errors
   `.text 0xNNN smaller than requested` — that 0xNNN IS the natural size. Set clip to it.
2. **Symbol extents**: `objdump -t` on the NM .o for the last symbol's start, plus the
   EXPECTED size of that symbol from `expected/<unit>.c.o` (not the NM one — it's the
   truncated victim). clip must be ≥ last_start + expected_size.
Sanity gate stays the same: after the rebuild, the exact-set diff must show the boundary fn
(62F08 name is `game_libs_func_00062F08`, NOT `gl_func_` — probe the right prefix) back at 100.

## A "caller-set $t6" cap preceded by a tiny no-jr-ra orphan is a hoisted-prologue mis-split (check libultra identity)
<a name="feedback-callerset-t6-orphan-head-is-hoisted-prologue"></a>

**Trigger (2026-07-17, gl_func_0006C2AC 74.96→100):** an NM wrap documented as a
"caller-set `$t6` gate" cap — the body compares `$t6` against a constant without
ever loading `$t6`. Before accepting the cap, look at the IMMEDIATELY PRECEDING
symbol: if it is a tiny orphan `.s` with **no `jr ra`** whose last insns SET that
register (here `game_libs_func_0006C2A4`, 0x8: `lui t6,%hi(D); lw t6,0xC(t6)`),
it is the function's own first load, **hoisted above the `addiu sp` prologue** by
the scheduler (IDO 5.3 -O1 does this; splat splits at the prologue). This is the
stolen-prologue class of `project_1080_orphan_fn_prologue_vein` meeting the
caller-set-`$t6` cap list — the existing rule "verify it's not a mis-split"
...
__osDisableInt). Sibling recipe = gl_func_0006F088 osLeoDiskInit donor:
IDO 5.3 -O1 donor TU, blank externs, TU-defined 2-byte `PgsRls` pair symbol
(`D_00000006_*` in undefined_syms_auto) so the pageSize/relDuration `sb` pair
batches under ONE `lui $at`, REPLACE_FUNC_BODY splice at the merged symbol,
merge the two `.s` (head words + body, size +8), targeted expected/-unit
refresh via temporary INCLUDE_ASM + splice-entry-removed build.
...
## NON_MATCHING_TEXT_CLIP: the LAST symbol must keep its FULL expected size — clipping it to a partial window drops a 100% sentinel to a length-ratio score (post0b 62F08 100->65, 2026-07-18 agent-f) <a name="clip-last-symbol-full-expected-size"></a>

When re-probing a post0b-style `NON_MATCHING_TEXT_CLIP_KEEP_ALIGN` after an NM body size change, do NOT compute the new clip as `old_clip + body_delta` if that lands inside the unit's LAST function. The tail function (game_libs_func_00062F08, expected size 0x50) scores 100 only when the NM object carries its full 0x50; a clip at `sym+0x34` triggers the auto-resize (0x50->0x34) and objdiff then scores 52-vs-80 bytes as 65% even though every byte matches. Fix: clip = last_sym_addr + full_expected_size (here 0x2b774+0x50 = 0x2b7c4, cutting only the 4 junk bytes past it). The `gl_func_000551E0=0x7c` style per-symbol pins are for jr-delay-nop parity and are independent of the tail rule. Also: an intermediate clip that truncates the tail symbol to 0 makes objdiff silently emit the function entry with NO `fuzzy_match_percent` key (shows as None) — that's the stale/over-clip fingerprint, not a report bug.
