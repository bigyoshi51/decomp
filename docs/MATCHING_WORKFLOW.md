...
- **GIANT MIS-SPLIT FUNCTION (high-value multi-tick merge target):** `func_80003D40` ends mid-computation (NO `jr ra`; tail is `lw t9, 0x3C(sp)`), and `func_80003E64`/`func_80003FF0` start with register-continuation insns (`or t2,t1,at` / `addu t3,t9,t2` — no prologue) → splat split ONE function into 4 fragments `func_80003D40 + func_80003E64 + func_80003FF0 + func_80004030` (~0x300 bytes; `func_80004030` holds the `0x800043D4` shared-tail branch target + the 2 `jr ra` returns). m2c fails on each fragment ("label .L800043D4 does not exist") because the branches cross the splat-imposed boundaries. FIX = merge the 4 `.s` into one `func_80003D40` symbol (merge-fragments x3 OR manual concat + grow the `nonmatching` size header + drop the 3 successor glabels), re-snapshot ONLY kernel_000.c.o per the make-expected trap, then lift the unified ~150-insn function. Big code-byte mover; worth a focused pass. (Check first whether 3E64/3FF0/4030 have external callers → keep them as `alabel`s if so.)
