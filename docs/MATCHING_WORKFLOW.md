...
2. **Plain `func_XXXX` names (bootup/kernel) unsupported.** The script's `MOD = FN.rsplit("_func_",1)[0]` needs a `<module>_func_` name; `func_000084A0` → MOD=`func_000084A0` → "no expected obj". Bootup needs manual disasm+m2c+wrap (or a script patch mapping bare `func_` → its segment).
3. **arcproc/etc. resolve calls to REAL names → arg-count conflicts.** Unlike game_uso (placeholder calls), m2c in arcproc emitted real `arcproc_uso_func_0000XXXX(...)` calls that clash with existing fixed-arg declarations ("number of arguments doesn't agree"). Fix: cast each such call to `((void(*)())FN)(...)` (bypasses the check, keeps the correct reloc) — but EXCLUDE the function's OWN definition line from the regex (casting the def line = syntax error; I hit this). 

These are tooling-cleanup tasks, not single-tick wins; a focused pass could patch the script for (2)+(3) and unlock the other-USO stub vein.
