...
- [1080 full-ROM misclassified-non-CPU sweep is COMPLETE (2026-08-22) — 31DF8 was the ONLY ucode/data region in the fn denominator; every other detector hit is handwritten CPU code or already-.rodata](#noncpu-sweep-complete-2026-08-22) — _All 236 remaining 0% fns scanned with `scripts/scan-noncpu-regions.py` (IMEM-jal / COP2-density / addi / kreg / mtc0 / ascii / mono-run / strict-decoder / branch-sanity). Zero new reclassifiable regions. Negatives table inside (exception handlers 0x6A5F0 + 800031F0, TLB/CP0 helpers, libgcc 64-bit shifts, kernel cross-branch fragments, kernel_026 .rodata already out of denominator). Don't re-run the sweep._

---

<a id="noncpu-sweep-complete-2026-08-22"></a>
## 1080 full-ROM misclassified-non-CPU sweep COMPLETE (2026-08-22) — 31DF8 was the only one; negatives banked

_2026-08-22 (agent-f), follow-up to the 31DF8 reclass. Question: are there MORE non-CPU regions (ucode/data) masquerading as 0% functions in any segment? Answer: **no**. All 236 remaining 0%-fuzzy functions (coverage-verified: every one has a scanned `.s`) were run through `scripts/scan-noncpu-regions.py` — two passes: (1) foreign-ISA signals (jal/j targets in SP IMEM 0x04001xxx, COP2/lwc2/swc2 density, `addi` [IDO never emits it], k0/k1/gp scratch, mtc0/mfc0, reserved opcodes, ASCII runs, monotone word runs), (2) strict o32+MIPS3 decoder validity + branch-target sanity + jr-ra presence. Only the already-reclassified `game_libs_rsp_ucode_31DF8` blob shows the ucode signature (imem_jal=52, 44% COP2/vector density, addi=136, 44% strict-decoder invalid). **Every other hit is genuine CPU code or already-declassified data:**_

| Region / fn | Signals that fired | Cleared as |
|---|---|---|
| game_libs 0x6A5F0 (478w) + 0x6AD78 (101w) | kreg=30/20, mtc0, mfc0, bad_op, mono_run≈24 | game_libs port of the libultra **exception handler** (`lui/addiu/jr $k0` vector, `sd $at..$ra` to $k0 save base, mfc0 Cause/SR, `ld` restore fan-in + `mtc0 $12`); mono_run = its embedded redispatch table; strict decoder ≈0% invalid. Handwritten CPU — permanent INCLUDE_ASM cap, NOT data. |
| kernel func_800031F0 (653w), func_80002E78 | same profile | kernel's own exception handler + CP0 preamble. Same class. |
| game_libs 0x6D894, kernel `__osProbeTLB`/800023E0/800062B0/800062D0/800099F0/9A50-ish, 0x70AC0/0x73258/0x6A09C | mtc0+mfc0 (+`addi` in handwritten src) | TLB probe (`tlbp`/`tlbr`) / SR-CP0 get-set helpers — handwritten libultra ports. CPU code. |
| game_libs 0x6C710 | bad_op=2 (dsrlv/dsll32/dsra32) | libgcc `__ull_rshift` 64-bit helper. CPU code. |
| kernel func_80002F78/800030D0/80003D40/80003E0C/80003E64/80008454 | badbr 25-75% (branch targets past declared end), 0% invalid opcodes | splat **fragments** of handwritten handlers (cross-fragment branches). CPU code; boundary issue, not data. |
| kernel func_80009EA0/func_80009EB0 (`kernel_026.c`) | ascii_run=10, weird_jal, pointer table 0x8000A5Dx + "USO: unloading %s"… strings | already `.section .rodata` + `dlabel` in the .s — **already outside the fn denominator** (report.json lists neither). No work needed. |

_Also negative: zero COP2/IMEM signals in ANY USO segment (boarder1-5, gui, all proc USOs, bootup, game) — the ucode class was game_libs-only. Takeaway: the fn-denominator is now clean of non-CPU code; future "0% resists everything" fns are caps, not misclassifications. Don't re-run this sweep; re-run the script only after a re-extract/refine-splat that adds new fns._
