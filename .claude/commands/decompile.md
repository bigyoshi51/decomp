...
- **A match = C compiles to the target bytes. Post-cc instruction-byte patching is BANNED** (INSN_PATCH / INSN_RELOC_PATCH / PROLOGUE_STEALS / instruction-appending SUFFIX_BYTES were removed 2026-05-23 — see `feedback_no_instruction_forcing_matches_policy`). If C can't be coaxed to the bytes (iterate shapes per `docs/IDO_CODEGEN.md`, or try the permuter), leave the function honestly `#ifdef NON_MATCHING` / `#else INCLUDE_ASM` — do NOT fake the match. `docs/POST_CC_RECIPES.md` is DEPRECATED (only genuine data/alignment mechanisms remain: all-zero padding SUFFIX, USO-header PREFIX_BYTES, TRUNCATE_TEXT).
...
1. **An existing NM wrap at 80-99%** — `grep -rn "#ifdef NON_MATCHING" src/`. Analysis is already done; a new technique (a different C shape, or the permuter) may turn it into a real match — i.e. C that *compiles* to the bytes. `report.json`'s `fuzzy_match_percent < 100` now means genuinely-not-matched (the old INSN_PATCH scoring quirk is gone — post-cc patching was removed 2026-05-23). To finish one, change the C so it compiles to the target; if you can't, leave it NON_MATCHING. Do NOT reach for byte-patching.
...
1a. **Boundary sanity check** — before grinding, verify the `.s` file contains ONE function. Splat/generate-uso-asm can mis-split boundaries in FOUR directions:
   ...
   **Quick pre-check:** `grep -c "03E00008" <asm_file>.s`. If the count is >1, the file contains multiple function bodies (each `jr $ra` ends one). For big strategy-memo picks, ALWAYS run this — the memo's "1.7 KB self-contained algorithm" label is unreliable on relocatable USO code because splat can't see function boundaries without symbol info.
   ...
   - **Prologue-stolen successor** (this `.s` starts with `addiu $sp` but immediately uses `$v0`/`$tN` as a base register without setting it): the implicit base-register setup (`lui rX,0; addiu rX,rX,N`) lives in the *predecessor's* symbol, so C-only emit duplicates it at the successor's start (+8 bytes vs expected). There is no clean C fix and **PROLOGUE_STEALS was removed (match-faking)** — leave the function `#ifdef NON_MATCHING` / `#else INCLUDE_ASM`. (Properly fixing it needs a splat boundary correction so the prefix bytes belong to the successor, not a post-cc splice.)
...
6. **Iterate if not matching — DO NOT GIVE UP EASILY**: Try at least 8-10 structurally different variations before moving on. Compile standalone first (`tools/gcc_2.7.2/linux/gcc -O2 -g2 -G0 -mips3 -mgp32 -mfp32 -Wa,--vr4300mul-off -S -o /tmp/test.s /tmp/test.c`) and compare the `.s` output against the original. This is faster than full ROM builds.
   ...
   - **Function emit is +8 bytes vs expected, with leading lui+addiu that doesn't appear in expected**: prologue-stolen successor — the implicit base-register setup lives in the predecessor's symbol. There's no clean C fix; PROLOGUE_STEALS (post-cc splice) was removed as match-faking. Leave it NON_MATCHING. See step 1a's "Prologue-stolen successor" boundary case.
